...
type
  TfrmAccBankaSubesi = class(TfrmBaseInputDB)
    lblbanka_id: TLabel;
    lblsube_kodu: TLabel;
    edtsube_kodu: TEdit;
    lblsube_adi: TLabel;
...
uses
  Ths.Globals, Ths.Database.Table.ChBankaSubeleri, ufrmAccBankalar,
  Ths.Database.Table.ChBankalar, ufrmSysSehirler, Ths.Database.Table.SysSehirler;

{$R *.dfm}

procedure TfrmAccBankaSubesi.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmAccBankaSubesi.FormDestroy(Sender: TObject);
begin
  inherited;
end;

procedure TfrmAccBankaSubesi.HelperProcess(Sender: TObject);
var
  LFrmSehirler: TfrmSysSehirler;
  LFrmBankalar: TfrmAccBankalar;
begin
  ...
  begin
    ...
    begin
      LFrmBankalar := TfrmAccBankalar.Create(TEdit(Sender), Self, TChBanka.Create(Table.Database), fomNormal, True);
      try
...
procedure TfrmAccBankaSubesi.RefreshData();
begin
...
procedure TfrmAccBankaSubesi.btnAcceptClick(Sender: TObject);
begin
  if (FormMode = ifmNewRecord) or (FormMode = ifmCopyNewRecord) or (FormMode = ifmUpdate) then
  begin
