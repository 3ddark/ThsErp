...
begin
  ...
  begin
    ...
    begin
      TRctPaketHammadde(Table).PaketAdi.Value := edtpaket_adi.Text;
...
begin
  ...
  if (pFormMode = ifmNewRecord) or (pFormMode = ifmCopyNewRecord) then
    Result := TfrmRctPaketHammaddeDetay.Create(Application, AGrid, Self, TRctPaketHammaddeDetay.Create(Table.Database), pFormMode)
  else if (pFormMode = ifmRewiev) or (pFormMode = ifmUpdate) then
  begin
    if Assigned(AGrid.Objects[COLUMN_GRID_OBJECT, AGrid.Row]) then
      Result := TfrmRctPaketHammaddeDetay.Create(Application, AGrid, Self, TRctPaketHammaddeDetay(AGrid.Objects[COLUMN_GRID_OBJECT, strngrd1.Row]), pFormMode);
  end;
end;
...
begin
  ...
  try
    ...
    LFirstHam := False;
    for n1 := 0 to TRctPaketHammadde(Table).ListDetay.Count-1 do
    begin
      if LFirstHam then
        strngrd1.Row := strngrd1.Row + 1;
      LFirstHam := True;

      strngrd1.Objects[COLUMN_GRID_OBJECT, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]);

      strngrd1.Cells[PH_MAL_KODU, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).StokKodu.Value;
      strngrd1.Cells[PH_MAL_ADI, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).StokAdi.Value;
      strngrd1.Cells[PH_MIKTAR, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).Miktar.Value;
      strngrd1.Cells[PH_BIRIM, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).OlcuBirimi.Value;
      strngrd1.Cells[PH_FIRE_ORANI, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).FireOrani.Value;
      strngrd1.Cells[PH_FIYAT, strngrd1.Row] := TRctPaketHammaddeDetay(TRctPaketHammadde(Table).ListDetay[n1]).Fiyat.Value;
    end;
...
begin
  ...
  edtpaket_adi.Text := TRctPaketHammadde(Table).PaketAdi.Value;
