...
begin
  ...
  begin
    ...
    begin
      TPrsEhliyet(Table).EhliyetID.Value := TSetPrsEhliyet(cbbdriver_license_id.Items.Objects[cbbdriver_license_id.ItemIndex]).Id.Value;
      TPrsEhliyet(Table).PersonelID.Value := TPrsPersonel(cbbemp_card_id.Items.Objects[cbbemp_card_id.ItemIndex]).Id.Value;
...
begin
  ...
  begin
    if Assigned(cbbemp_card_id.Items.Objects[n1]) then
      if TPrsPersonel(cbbemp_card_id.Items.Objects[n1]).Id.Value = TPrsEhliyet(Table).PersonelID.Value then
      begin
  ...
  cbbdriver_license_id.ItemIndex := cbbdriver_license_id.Items.IndexOf(TPrsEhliyet(Table).Ehliyet.Value);
end;
