...
begin
  ...
  FIsSatilabilir := TFieldDB.Create('is_satilabilir', ftBoolean, False, Self);
  FStokKodu := TFieldDB.Create('stok_kodu', ftString, '', Self);
  FStokAdi := TFieldDB.Create('stok_adi', ftString, '', Self);
  FStokGrubuID := TFieldDB.Create('stok_grubu_id', ftInteger, 0, Self);
  FStokGrubu := TFieldDB.Create(FStkStokGrubu.Grup.FieldName, FStkStokGrubu.Grup.DataType, '', Self);
  ...
  FUrunTipi := TFieldDB.Create('urun_tipi', ftSmallInt, 0, Self);
  FAlisIskonto := TFieldDB.Create('alis_iskonto', ftFloat, 0, Self);
  FSatisIskonto := TFieldDB.Create('satis_iskonto', ftFloat, 0, Self);
  FAlisFiyat := TFieldDB.Create('alis_fiyat', ftFloat, 0, Self);
  FAlisPara := TFieldDB.Create('alis_para', ftString, '', Self);
  FSatisFiyat := TFieldDB.Create('satis_fiyat', ftFloat, 0, Self);
  FSatisPara := TFieldDB.Create('satis_para', ftString, '', Self);
  FIhracFiyat := TFieldDB.Create('ihrac_fiyat', ftFloat, 0, Self);
  FIhracPara := TFieldDB.Create('ihrac_para', ftString, '', Self);
  FOrtalamaMaliyet := TFieldDB.Create('ortalama_maliyet', ftFloat, 0, Self);
  ...
  FAgirlik := TFieldDB.Create('agirlik', ftFloat, 0, Self);
  FTeminSuresi := TFieldDB.Create('temin_suresi', ftInteger, 0, Self);
  FOzelKod := TFieldDB.Create('ozel_kod', ftString, '', Self);
  FMarka := TFieldDB.Create('marka', ftString, '', Self);
  FMenseiID := TFieldDB.Create('mensei_id', ftInteger, 0, Self);
  FMensei := TFieldDB.Create(FSysUlke.UlkeAdi.FieldName, FSysUlke.UlkeAdi.DataType, '', Self);
  FGtipNo := TFieldDB.Create('gtip_no', ftString, '', Self);
  FDiibUrunTanimi := TFieldDB.Create('diib_urun_tanimi', ftString, '', Self);
  FEnAzStokSeviyesi := TFieldDB.Create('en_az_stok_seviyesi', ftFloat, 0, Self);
  FTanim := TFieldDB.Create('tanim', ftString, '', Self);
  FCins := TFieldDB.Create(FStkCinsOzellik.Cins.FieldName, FStkCinsOzellik.Cins.DataType, '', Self);
