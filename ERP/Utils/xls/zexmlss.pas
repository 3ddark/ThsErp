//****************************************************************
// ZEXMLSS  (Z Excel XML SpreadSheet) - non-visual component,
// designed to save data in Excel XML format.
// Created in Mozyr in 2009
// Author: Ruslan V. Neborak
// e-mail: avemey(at)tut(dot)by
//****************************************************************
// zexmlss.pas - Core data model (TZEXMLSS, TZSheet, TZCell, TZStyle...)
// Based on ZEXMLSS by Ruslan V. Neborak (Avemey) - https://github.com/Avemey/zexmlss
// Modernized for Delphi 10.4 Sydney and above
// Changes:
//   - FPC / Lazarus support removed
//   - Legacy Delphi (5/6/7/XE/XE2) compatibility removed
//   - uses: WinApi.Windows (was conditional), LCLType/LResources removed
//   - {$IFDEF FPC} blocks removed
//   - Deprecated SaveToFile/SaveToStream (ansistring + CodePage) removed
//   - UTF-8 is the only supported encoding
//   - ZUSE_CONDITIONAL_FORMATTING / ZUSE_CHARTS / ZUSE_DRAWINGS preserved
//****************************************************************
// URL:    http://avemey.com
// Ver:    0.0.8
// License: zlib
...
uses
  System.Classes, System.SysUtils, System.UITypes, Vcl.Graphics, Winapi.Windows,
  zsspxml;

const
  _PointToMM: real = 0.3528;  // 1 typographic point = 0.3528 mm
...
const
  ZEAnsiString = ZEString deprecated 'Use ZEString instead'; // backward compatibility
...
type
  ...
  private
    ...
    function GetDataAsBoolean: Boolean;
    procedure SetDataAsBoolean(const Value: Boolean);
  public
    constructor Create(); virtual;
    procedure Assign(Source: TPersistent); override;
    procedure Clear();
    property AlwaysShowComment: boolean read FAlwaysShowComment write FAlwaysShowComment default false;
    property Comment: string read FComment write FComment;      //comment text
    ...
    property CellType: TZCellType read FCellType write FCellType default ZEString; //Cell data type
    property Data: string read FData write FData;
    property Formula: string read FFormula write FFormula;     //formula in R1C1 style1
    property HRef: string read FHref write FHref;              //hyperlink
    ...
    property AsBoolean: Boolean read GetDataAsBoolean write SetDataAsBoolean;
  end;
  ...
  //   2 - Right border
  //   3 - Bottom border
  //   4 - DiagonalLeft   diagonal from top-left to bottom-right
  //   5 - DiagonalRight  diagonal from bottom-left to top-right
  ...
  /// Text rotation angle in cell. Signed integer.
  ///    Base range -90..+90, extended -180..+180 (degrees)
  ///    Extended range values CANNOT work in Microsoft Excel!
  /// XML SS definition matches base range
  /// OpenDocument 1.2 allows any fractional number
  ///    in different units (degrees, gradians, radians).
  ///    practically uses integer from 0 to 359
  /// OfficeXML definition is most complex:
  ///    254 and 255 are special values,
  ///        255 works in Excel though not documented in the standard
  ///        254 is accepted but does not work after opening the file
  ///    0 to +90 matches both standards described above
  ///    +91 to +180 corresponds to -1..-90 [ = 90 - value]
  ///
  ...
  //alignment
        //ReadingOrder - probably not needed
  ...
  private
    ...
    FShrinkToFit: boolean;
                                        //   fits in the cell,
                                        // false - text is not shrunk
    FVertical: TZVerticalAlignment;     //Vertical alignment
...
// introduced don't know when in FPC
  ...
  private
    // Inches are annoying! Use MM instead!
    // Note: maybe use Cardinal instead of word?
    FActiveCol: word;
    FActiveRow: word;
    FMarginBottom: word;
    FMarginLeft: word;
    ...
    FSplitVerticalValue: integer;
    FSplitHorizontalValue: integer;
                                        //    in pixels if SplitMode = ZSplitSplit
                                        //    in row/column count if SplitMode = ZSplitFrozen
                                        // If SplitMode = ZSplitNone, row/column freeze is disabled
  ...
  //Operator for conditional formatting
  ...
  //Conditional formatting: style per condition
  TZConditionalStyleItem = class(TPersistent)
  private
    FCondition: TZCondition;
    FConditionOperator: TZConditionalOperator;
    FValue1: string;
    FValue2: string;
    FApplyStyleID: integer;

                                                //Base cell (for formulas only):
    FBaseCellPageIndex: integer;
                                                //    -1 - current sheet
    FBaseCellRowIndex: integer;
    FBaseCellColumnIndex: integer;
  protected
  public
    constructor Create(); virtual;
    procedure Clear();
  ...
  //Area for conditional formatting
  ...
  //Areas for conditional formatting
  ...
  //Conditional formatting: condition list
  ...
  TZEChartType = (ZEChartTypeArea, ZEChartTypeBar, ZEChartTypeBubble, ZEChartTypeCircle, ZEChartTypeGantt, ZEChartTypeLine, ZEChartTypeRadar, ZEChartTypeRing, ZEChartTypeScatter, ZEChartTypeStock, ZEChartTypeSurface);
  ...
  public
    ...
    //TODO: move save/load functions to a separate module
    ...
    // Iterates all sheets and returns Drawing by global sequential index
    // as if there is no sheet separation
    function GetDrawing(num: Integer): TZEDrawing;
    // returns sheet index for the given Drawing
  ...
  published
    ...
    property HorPixelSize: real read FHorPixelSize write SetHorPixelSize;
    property VertPixelSize: real read FVertPixelSize write SetVertPixelSize;
  end;

procedure Register();

//Validates and corrects a string for XML
procedure CorrectStrForXML(const St: string; var Corrected: string; var UseXMLNS: boolean);

//Converts TColor to Hex RGB string
function ColorToHTMLHex(Color: TColor): string;

//Converts Hex RGB string to TColor
...
//Converts date to XML string (YYYY-MM-DDTHH:MM:SS[.mmm])
...
//Converts integer to string with minimum NullCount digits
//TODO: check functions in FlyLogReader
//INPUT
//      value: integer     - the number
//      NullCount: integer - minimum digits in result string
//RETURN
//      string
...
//Converts date to XML string (YYYY-MM-DDTHH:MM:SS[.mmm])
//INPUT
//      ATime: TDateTime - the date/time value
...
//      var Corrected: string - corrected string
//      var UseXMLNS: boolean - whether string has valid paired XML tags
//                              true - has valid tags
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    Stack[kol - 2].RawTag := Stack[kol - 2].RawTag + Stack[kol - 1].TextBeforeTag + Stack[kol - 1].RawTag + Stack[kol].TextBeforeTag + Stack[kol].RawTag;
                    Corrected := '';
                  end
                  else
                    Corrected := Corrected + Stack[kol - 1].TextBeforeTag + Stack[kol - 1].RawTag + Stack[kol].TextBeforeTag + Stack[kol].RawTag;
                  dec(t);
                  UseXMLNS := true;
                  dec(kol);
                  SetLength(Stack, kol);
...
begin
  ...
  begin
    ...
    if value[1] = '#' then
      delete(value, 1, 1);
    //TODO: handle named colors like "black"?
...
//Convert pixels to typographic points
//Input
//      inPixel: integer          - size in pixels
//      PixelSizeMM: real = 0.265 - pixel size in mm
function PixelToPoint(inPixel: integer; PixelSizeMM: real = 0.265): real;
begin
...
begin
  case pp of
    ...
    ZPVertStripe:
      result := 'VertStripe';
    ZPReverseDiagStripe:
      result := 'ReverseDiagStripe';
    ZPDiagStripe:
      result := 'DiagStripe';
    ZPDiagCross:
      result := 'DiagCross';
    ZPThickDiagCross:
      result := 'ThickDiagCross';
    ZPThinHorzStripe:
      result := 'ThinHorzStripe';
    ZPThinVertStripe:
      result := 'ThinVertStripe';
    ZPThinReverseDiagStripe:
      result := 'ThinReverseDiagStripe';
    ZPThinDiagStripe:
      result := 'ThinDiagStripe';
    ZPThinHorzCross:
      result := 'ThinHorzCross';
    ZPThinDiagCross:
      result := 'ThinDiagCross';
  end;
end;
...
//Returns:
//     -2          - style not found
//     -1          - matches default style
//      0..Count-1 - style index
...
//      CheckMatch - whether to check for duplicates
//Returns:
//      Index of the added (or existing duplicate) style
...
//Delete style at index num, higher-indexed styles shift down
// num - style index
//Returns:
//       0  - deleted successfully
//      -1  - could not delete style
...
begin
  ...
  begin
    ...
    // When deleting: update cells referencing num to 0, shift others.
    result := 0;
  end
  else
    result := -1;
...
function TZCell.GetDataAsBoolean: Boolean;
var
  b: Boolean;
begin
  b := false;
  ...
  else if (not TryStrToBool(FData, Result)) then
  begin
      //If cell type is number then try convert "float" to datetime
    if (CellType = ZEBoolean) then
      Result := AsBoolean
    else
      b := true;
  end;
  ...
  if (b) then
    raise EConvertError.Create('ZxCell: Cannot cast data to Boolean');
  ;
end;
...
procedure TZCell.SetDataAsBoolean(const Value: Boolean);
begin
  FCellType := ZEBoolean;
...
begin
  FData := StringReplace(FloatToStr(Value), ',', '.', [rfReplaceAll]);
  CellType := ZENumber;
end;
...
//Merge cells within rectangle
//accepts rectangle (Rct: TRect)
//returns:
//      0 - OK, cells merged
//      1 - rectangle out of bounds, cells not merged
//      2 - rectangle intersects existing merge areas, cells not merged
//      3 - single-cell rectangle not added
...
var
  ...
  begin
    result := (((rct1.Left >= rct2.Left) and (rct1.Left <= rct2.Right)) or ((rct1.right >= rct2.Left) and (rct1.right <= rct2.Right))) and (((rct1.Top >= rct2.Top) and (rct1.Top <= rct2.Bottom)) or ((rct1.Bottom >= rct2.Top) and (rct1.Bottom <= rct2.Bottom)));
  end;
...
begin
  ...
  //Expand row/column storage if needed
  if (FSheet <> nil) then
  begin
...
//Check if cell is the top-left corner of a merge area (
//accepts (ACol, ARow: integer) - cell coordinates
//Returns:
//      -1              cell is NOT the top-left corner of any merge area
//      int num >=0     index of merge area where cell is top-left corner
...
//Check if cell is inside a merge area
//accepts (ACol, ARow: integer) - cell coordinates
//Returns:
//      -1              cell is NOT in any merge area
//      int num >=0     index of merge area containing the cell
...
begin
  result := -1;
  for i := 0 to FCount - 1 do
    if (ACol >= FMergeArea[i].Left) and (ACol <= FMergeArea[i].Right) and (ARow >= FMergeArea[i].Top) and (ARow <= FMergeArea[i].Bottom) then
    begin
      result := i;
      break;
    end;
...
//Delete merge area at index num
//Returns:
//      - TRUE  - area deleted
//      - FALSE - num out of range
...
//Merge cells within rectangle
//accepts rectangle coordinates (x1, y1, x2, y2: integer)
//returns:
//      0 - OK, cells merged
//      1 - rectangle out of bounds, cells not merged
//      2 - rectangle intersects existing merge areas, cells not merged
...
begin
  ...
  begin
    _t := Source as TZSheetFooterHeader;
    result := (_t.IsDisplay = FIsDisplay) and (_t.DataLeft = FDataLeft) and (_t.Data = FData) and (_t.DataRight = FDataRight);
  end;
end;
...
begin
  ...
  if (Assigned(Source)) then
    ...
    begin
      t := Source as TZHeaderFooterMargins;
      result := (FMarginTopBottom = t.MarginTopBottom) and (FMarginLeft = t.MarginLeft) and (FMarginRight = t.MarginRight) and (FHeight = t.FHeight) and (FUseAutoFitHeight = t.UseAutoFitHeight);
    end;
end;
...
begin
  ...
  begin
    ...
    {TODO: add more fields to copy if needed}
...
//Get HorPixelSize and VertPixelSize for device hdc
procedure TZEXMLSS.GetPixelSize(hdc: HWND);
begin
  HorPixelSize := GetDeviceCaps(hdc, HORZSIZE) / GetDeviceCaps(hdc, HORZRES); //horizontal pixel size in millimeters
...
//Saves to file FileName
//      SheetsNumbers: array of integer  - sheet index array in desired order
//      SheetsNames: array of string     - sheet name array
//              element count in both arrays must match
//      CodePage: byte                   - encoding (deprecated, UTF-8 only)
//                                         0 - UTF-8
//                                         1 - windows-1251
{var
  Stream: TStream;
...
begin
  {tut}
  //TODO: should there be a max count limit?
  if (value >= 0) then
  begin
...
//Determines if cell is inside a conditional formatting area
//INPUT
//      ColumnNum: integer  - cell column number
//      RowNum: integer     - cell row number
//RETURN
//      boolean - true if cell is inside the area
...
//Add conditional formatting with area
//INPUT
//      ColumnNum: integer  - column number
//      RowNum: integer     - row number
//      AreaWidth: integer  - area width
//      AreaHeight: integer - area height
//RETURN
//      TZConditionalStyle - the added style
...
begin
  ...
  begin
    tmp := Source as TZETransform;
    Result := (FRotate = tmp.Rotate) and (FScaleX = tmp.ScaleX) and (FScaleY = tmp.ScaleY) and (FSkewX = tmp.SkewX) and (FSkewY = tmp.SkewY) and (FTranslateX = tmp.TranslateX) and (FTranslateY = tmp.TranslateY);
  end;
end; //IsEqual
...
begin
  ...
  begin
    ...
    begin
      tmp := Source as TZEChartRangeItem;
      Result := (FSheetNum = tmp.SheetNum) and (FCol = tmp.Col) and (FRow = tmp.Row) and (FHeight = tmp.Height) and (FWidth = tmp.Width);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      tmp := Source as TZEChartAxis;
      Result := (FLogarithmic = tmp.FLogarithmic) and (FReverseDirection = tmp.FReverseDirection) and (FAutoScaleMin = tmp.AutoScaleMin) and (FAutoScaleMax = tmp.AutoScaleMax);
      if (Result) then
      begin
...
begin
  ...
  if (Result) then
    ...
    begin
      ...
      if (Result) then
        Result := FSubtitle.IsEqual(tmp.Subtitle) and FTitle.IsEqual(tmp.Title) and FLegend.IsEqual(tmp.Legend) and FAxisX.IsEqual(tmp.AxisX) and FAxisY.IsEqual(tmp.AxisY) and FAxisZ.IsEqual(tmp.AxisZ) and FSecondaryAxisX.IsEqual(tmp.SecondaryAxisX) and FSecondaryAxisY.IsEqual(tmp.SecondaryAxisY) and FSecondaryAxisZ.IsEqual(tmp.SecondaryAxisZ) and (FView3D = tmp.View3D) and (FViewDeep = tmp.ViewDeep);
    end;
end;
...
begin
  ...
  begin
    tmp := Source as TZEPicture;
    Result := (FId = tmp.Id) and (FTitle = tmp.Title) and (FDescription = tmp.Description) and (FHidden = tmp.Hidden);
...
{
GetDeviceCaps(hdc, HORZSIZE) / GetDeviceCaps(hdc, HORZRES); //horizontal pixel size in millimeters
GetDeviceCaps(hdc, VERTSIZE) / GetDeviceCaps(hdc, VERTRES); //vertical pixel size in millimeters
}
