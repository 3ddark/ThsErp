...
type
  TfrmAccDovizKurlari = class(TfrmBaseDBGrid)
  protected
...
function TfrmAccDovizKurlari.CreateInputForm(Sender: TObject; pFormMode: TInputFormMode): TForm;
begin
  Result := nil;
  if (pFormMode = ifmRewiev) then
    Result := TfrmAccDovizKuru.Create(Self, Self, Table.Clone, pFormMode)
  else if (pFormMode = ifmNewRecord) then
    Result := TfrmAccDovizKuru.Create(Self, Self, TChDovizKuru.Create(Table.Database), pFormMode)
  else if (pFormMode = ifmCopyNewRecord) then
    Result := TfrmAccDovizKuru.Create(Self, Self, Table.Clone, pFormMode);
end;

procedure TfrmAccDovizKurlari.FormShow(Sender: TObject);
var
  LHane: Integer;
  Lfs: TFormatSettings;
begin
