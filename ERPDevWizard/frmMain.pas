...
//    con1.LibraryLocation := ExtractFilePath(Application.ExeName) + PathDelim + 'lib' + PathDelim + 'libpq.dll';
    try
      ...
      ShowMessage('Veri taban� ile ba�lant� kurulmad�! L�tfen ayarlar�n�z� kontrol edin.');
    end;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    try
      ...
      //projede kullan�lan dosyalardan sonraki son sat�r numaras�n� bul
      n1 := vFileDPR.IndexOf('{$R *.res}');

      //son eleman�n noktal� virg�l bilgisini virg�le �evir.
      vFileDPR.Strings[n1-2] := LeftStr(vFileDPR.Strings[n1-2], Length(vFileDPR.Strings[n1-2])-1) + ',';

      //eklenen s�n�f, output ve input formlar�n� projeye dahil et
