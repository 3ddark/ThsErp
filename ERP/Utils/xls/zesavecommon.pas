...
// Validates sheet headers, corrects if necessary
function ZECheckTablesTitle(var XMLSS: TZEXMLSS; const SheetsNumbers: array of integer; const SheetsNames: array of string; out _pages: TIntegerDynArray; out _names: TStringDynArray; out retCount: integer): boolean;
...
begin   // todo: compiler version and date ?
  ...
  {$WARNINGS OFF}
  if ZELibraryFork <> '' then
    Result := Result + '@' + ZELibraryFork;
  {$WARNINGS ON}

  Result := Result + '$' + 'DELPHI_or_CBUILDER';
  Result := Result + ' ZEXMLSS/xls-pas-lib';
end;
...
//INPUT
//   var DirName: string - directory path
//RETURN
//      boolean - true - OK
...
// Convert string to boolean
//INPUT
//   const val: string - string to convert
...
// Try to convert string to boolean
//   const st: string        - string to parse
//   valueIfError: boolean - fallback value on parse error
function ZETryStrToBoolean(const st: string; valueIfError: boolean = false): boolean;
begin
  result := valueIfError;
  if (st > '') then
...
begin
  ...
  Result := 'PT' + IntToStr(_h) + 'H' + IntToStr(_min) + 'M' + ZEFloatSeparator(FloatToStr(_sec + _msec / 1000)) + 'S';
