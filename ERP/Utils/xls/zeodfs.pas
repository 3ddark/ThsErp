...
uses
  System.SysUtils, Vcl.Graphics, System.Classes, System.Types, System.UITypes,
  zsspxml, zexmlss, zesavecommon, zenumberformats, zeZipper;

type
  TZEConditionMap = record
    ConditionValue: string;
    ApplyStyleName: string;
    ApplyStyleIDX: integer;
    ApplyBaseCellAddres: string
  end;
  ...
  TZEODFStyleProperties = record
    ...
    Conditions: array of TZEConditionMap;
  end;
  ...
  private
    ...
    FAreas: array of TZEODFCFAreaItem;
    FAreasCount: integer;
    FMaxAreasCount: integer;

    FLineItemWidth: integer;
    FLineItemStartCell: integer;
    FLineItemStyleCFNumber: integer;
    FReadHelper: TZEODFReadHelper;
  protected
    procedure AddToLine(const CellNum: integer; const AStyleCFNumber: integer; const ACount: integer);
    function ODFReadGetConditional(const ConditionalValue: string; out Condition: TZCondition; out ConditionOperator: TZConditionalOperator; out Value1: string; out Value2: string): boolean;
  public
    constructor Create(XMLSS: TZEXMLSS);
    destructor Destroy(); override;
    procedure CheckCell(CellNum: integer; AStyleCFNumber: integer; RepeatCount: integer = 1);
    procedure ApplyConditionStylesToSheet(SheetNumber: integer; var DefStylesArray: TZODFStyleArray; DefStylesCount: integer; var StylesArray: TZODFStyleArray; StylesCount: integer);
  ...
  TODFCFWriterArray = record
    CountCF: integer;
    StyleCFID: array of TODFStyleCFID;
    Areas: TODFCFAreas;
  end;
  ...
  private
    ...
    FApplyCFStylesCount: integer;
    FMaxApplyCFStylesCount: integer;
    FApplyCFStyles: array of integer;
  protected
    function GetBaseCellAddr(const StCondition: TZConditionalStyleItem; const CurrPageName: string): string;
    function AddBetweenCond(const ConditName, Value1, Value2: string; out retCondition: string): boolean;
  public
    constructor Create(ZEXMLSS: TZEXMLSS; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer);
  ...
  TODSManifestMediaType = (ZEODSMediaTypeSpreadSheet, ZEODSMediaTypeTextXml, ZEODSMediaTypeRdfXml, ZEODSMediaTypeConfig, ZEODSMediaTypeImagePng, ZEODSMediaTypeChart, ZEODSMediaTypeGdiMetaFile, ZEODSMediaTypeNull, ZEODSMediaTypeUnknown);
  ...
  private
    ...
    FConditionReader: TZODFConditionalReadHelper;
  ...
  protected
  public
    constructor Create(AXMLSS: TZEXMLSS; const _pages: TIntegerDynArray; const _names: TStringDynArray; PagesCount: integer); reintroduce; overload; virtual;
    destructor Destroy(); override;
...
function SaveXmlssToODFSPath(var XMLSS: TZEXMLSS; PathName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;

function SaveXmlssToODFSPath(var XMLSS: TZEXMLSS; PathName: string): integer; overload;

function ExportXmlssToODFS(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;
...
function ODFCreateStyles(var XMLSS: TZEXMLSS; Stream: TStream; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer; const WriteHelper: TZEODFWriteHelper): integer;

function ODFCreateSettings(var XMLSS: TZEXMLSS; Stream: TStream; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer): integer;

function ODFCreateContent(var XMLSS: TZEXMLSS; Stream: TStream; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer; const WriteHelper: TZEODFWriteHelper): integer;

function ODFCreateMeta(var XMLSS: TZEXMLSS; Stream: TStream): integer;
...
const
  ...
  const_ODS_paper_sizes: array[0..const_ODS_paper_sizes_count - 1] of array[0..1] of integer = ((0, 0),             // 0        Undefined
  ...
  const_ODS_manifest_mediatypes_str: array[0..7] of string = ('application/vnd.oasis.opendocument.spreadsheet',  //ZEODSMediaTypeSpreadSheet
  ...
  const_ODS_manifest_mediatypes: array[0..8] of TODSManifestMediaType = (ZEODSMediaTypeSpreadSheet, ZEODSMediaTypeTextXml, ZEODSMediaTypeRdfXml, ZEODSMediaTypeConfig, ZEODSMediaTypeImagePng, ZEODSMediaTypeChart, ZEODSMediaTypeGdiMetaFile, ZEODSMediaTypeNull, ZEODSMediaTypeUnknown);
...
constructor TZODFConditionalWriteHelper.Create(ZEXMLSS: TZEXMLSS; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer);
var
  i: integer;
begin
  FStylesCount := 0;
...
function TZODFConditionalWriteHelper.GetBaseCellAddr(const StCondition: TZConditionalStyleItem; const CurrPageName: string): string;
var
  s: string;
  b: boolean;
  i: integer;
...
begin
  ...
  begin
    ...
    begin
      result := true;
      retCondition := ConditName + '(' + ZEFloatSeparator(FormatFloat('', t1)) + ',' + ZEFloatSeparator(FormatFloat('', t2)) + ')'
    end;
  end;
end; //AddBetweenCond
...
var
  ...
  var
    ...
    var
      ...
      begin
        ...
        case (StyleItem.Condition) of
          ZCFIsTrueFormula:
            result := _GetIsTrueFormula(retCondition);
          ZCFCellContentIsBetween:
            result := AddBetweenCond('between', StyleItem.Value1, StyleItem.Value2, retCondition);
          ZCFCellContentIsNotBetween:
            result := AddBetweenCond('not-between', StyleItem.Value1, StyleItem.Value2, retCondition);
          ZCFCellContentOperator:
            result := _GetContentOperator(retCondition);
          ZCFNumberValue:
            ;
          ZCFString:
            ;
          ...
          ZCFFormula:
            ;
          ZCFContainsText:
            result := _GetTextCondition('contains-text', retCondition);
          ZCFNotContainsText:
            result := _GetTextCondition('not-contains-text', retCondition);
          ZCFBeginsWithText:
            result := _GetTextCondition('begins-with', retCondition);
          ZCFEndsWithText:
            result := _GetTextCondition('ends-with', retCondition);
          ZCFCellIsEmpty:
            ;
          ZCFDuplicate:
            result := _GetSimpleText('duplicate', retCondition);
          ZCFUnique:
            result := _GetSimpleText('unique', retCondition);
          ZCFAboveAverage:
            result := _GetSimpleText('above-average', retCondition);
          ZCFBellowAverage:
            result := _GetSimpleText('below-average', retCondition);
          ZCFAboveEqualAverage:
            result := _GetSimpleText('above-equal-average', retCondition);
          ZCFBelowEqualAverage:
            result := _GetSimpleText('below-equal-average', retCondition);
          ZCFTopElements:
            result := _GetConditionOneNumber('top-elements', retCondition);
          ZCFBottomElements:
            result := _GetConditionOneNumber('bottom-elements', retCondition);
          ZCFTopPercent:
            result := _GetConditionOneNumber('top-percent', retCondition, true);
          ZCFBottomPercent:
            result := _GetConditionOneNumber('bottom-percent', retCondition, true);
          ZCFIsError:
            result := _GetSimpleText('is-error', retCondition);
          ZCFIsNoError:
            result := _GetSimpleText('is-no-error', retCondition);
        end;
      end; //_GetCondition
    ...
    begin
      ...
      for i := 0 to n do
      begin
        result := result + s + '.' + ZEGetA1byCol(_StyleItem.Areas[i].Column) + IntToStr(_StyleItem.Areas[i].Row + 1) + ':' + s + '.' + ZEGetA1byCol(_StyleItem.Areas[i].Column + _StyleItem.Areas[i].Width - 1) + IntToStr(_StyleItem.Areas[i].Row + _StyleItem.Areas[i].Height);
        if (i <> n) then
          result := result + ' ';
      end;
    end; //_GetRanges
...
function TZODFConditionalReadHelper.ODFReadGetConditional(const ConditionalValue: string; out Condition: TZCondition; out ConditionOperator: TZConditionalOperator; out Value1: string; out Value2: string): boolean;
var
  i: integer;
  len: integer;
  s: string;
...
procedure TZODFConditionalReadHelper.ApplyConditionStylesToSheet(SheetNumber: integer; var DefStylesArray: TZODFStyleArray; DefStylesCount: integer; var StylesArray: TZODFStyleArray; StylesCount: integer);
var
  i, j: integer;
  _CFCount: integer;
  ...
  begin
    for i := 0 to StyleItem.ConditionsCount - 1 do
      if (ODFReadGetConditional(StyleItem.Conditions[i].ConditionValue, _Condition, _ConditionOperator, _Value1, _Value2)) then
      begin
  ...
  begin
    _CF.Items[NewCFNumber].Areas.Add(FAreas[AreaNumber].ColNum, FAreas[AreaNumber].RowNum, FAreas[AreaNumber].Width, FAreas[AreaNumber].Height);
  end; //_AddArea
...
var
  ...
  begin
    ...
    if (_CFvalue <> '') then
      if (ODFReadGetConditional(_CFvalue, _Condition, _ConditionOperator, _Value1, _Value2)) then
      begin
        num := _CFItem.Count;
        _CFItem.Add();
...
begin
  ...
  (*
   <calcext:conditional-format calcext:target-range-address="Sheet1.A1:Sheet1.D17 Sheet1.E1:Sheet1.F17">
      <calcext:condition calcext:apply-style-name="Unnamed1" calcext:value="begins-with(&quot;as&quot;)" calcext:base-cell-address="Sheet1.A1"/>
      <calcext:condition calcext:apply-style-name="Unnamed2" calcext:value="ends-with(&quot;ey&quot;)" calcext:base-cell-address="Sheet1.A1"/>
      <calcext:condition calcext:apply-style-name="Unnamed3" calcext:value="contains-text(&quot;et&quot;)" calcext:base-cell-address="Sheet1.A1"/>
      <calcext:condition calcext:apply-style-name="Unnamed4" calcext:value="not-contains-text(&quot;rt&quot;)" calcext:base-cell-address="Sheet1.A1"/>
      <calcext:condition calcext:apply-style-name="Unnamed5" calcext:value="between(1,20)" calcext:base-cell-address="Sheet1.A1"/>
...
constructor TZEODFWriteHelper.Create(AXMLSS: TZEXMLSS; const _pages: TIntegerDynArray; const _names: TStringDynArray; PagesCount: integer);
var
  i: integer;
  _a: array of integer;
  ...
  begin
    ...
    begin
      ...
      b := (_SO.MarginLeft = SheetOptions.MarginLeft) and (_SO.MarginBottom = SheetOptions.MarginBottom) and (_SO.MarginTop = SheetOptions.MarginTop) and (_SO.MarginRight = SheetOptions.MarginRight) and (_SO.PaperSize = SheetOptions.PaperSize) and (_SO.PortraitOrientation = SheetOptions.PortraitOrientation) and (_SO.HeaderMargins.IsEqual(SheetOptions.HeaderMargins)) and (_SO.FooterMargins.IsEqual(SheetOptions.FooterMargins)) and (_SO.HeaderBGColor = SheetOptions.HeaderBGColor) and (_SO.FooterBGColor = SheetOptions.FooterBGColor);
      if (b) then
      begin
        Result := i;
        exit;
  ...
  begin
    ...
    begin
      ...
      b := (_SO.Header.IsEqual(SheetOptions.Header)) and (_SO.Footer.IsEqual(SheetOptions.Footer)) and (_SO.IsEvenFooterEqual = SheetOptions.IsEvenFooterEqual) and (_SO.IsEvenHeaderEqual = SheetOptions.IsEvenHeaderEqual) and (FPageLayoutsIndexes[FMasterPages[i + 1]] = _a[PageNum]);
      if (b) then
      begin
        Result := i;
        exit;
  ...
  begin
    ...
    begin
      ...
      begin
        if (s = '0pt') then
          BStyle.Weight := 0
        else if (s = '0.26pt') then
          BStyle.Weight := 1
        else if (s = '2.49pt') then
          BStyle.Weight := 2
        else if (s = '4pt') then
          BStyle.Weight := 3
        else
...
begin
{
  ...
  begin
    n := 4;
    if (not (((ProcessedStyle.Border[0].LineStyle = ZEContinuous) or (ProcessedStyle.Border[0].LineStyle = ZEHair)) and (ProcessedStyle.Border[0].Weight = 0))) then
    begin
  ...
  begin
    case (j) of
      ...
      1:
        satt := 'fo:border-top';
      2:
        satt := 'fo:border-right';
      3:
        satt := 'fo:border-bottom';
      4:
        satt := 'style:diagonal-bl-tr';
      5:
        satt := 'style:diagonal-tl-br';
    end;
    if (not (((ProcessedStyle.Border[j].LineStyle = ZEContinuous) or (ProcessedStyle.Border[j].LineStyle = ZEHair)) and (ProcessedStyle.Border[j].Weight = 0))) then
    begin
  ...
  case (ProcessedStyle.Alignment.Vertical) of
    ...
    ZVBottom:
      s := 'bottom';
    ZVCenter, ZVDistributed, ZVJustifyDistributed:
      s := 'middle';
    //(top | middle | bottom | automatic)
  end;
  _xml.Attributes.Add('style:vertical-align', s);

  _xml.Attributes.Add('style:text-align-source', IfThen(ZHAutomatic = ProcessedStyle.Alignment.Horizontal, 'value-type', 'fix'));

  if ZHFill = ProcessedStyle.Alignment.Horizontal then
    _xml.Attributes.Add('style:repeat-content', ODFBoolToStr(true), false);

  _xml.Attributes.Add('style:direction', IfThen(ProcessedStyle.Alignment.VerticalText, 'ttb', 'ltr'), false);
  if (ProcessedStyle.Alignment.Rotate <> 0) then
    _xml.Attributes.Add('style:rotation-angle', IntToStr(ProcessedStyle.Alignment.Rotate mod 360));
  ...
  if ZHAutomatic <> ProcessedStyle.Alignment.Horizontal then
  begin
    _xml.Attributes.Clear();
    ...
    case (ProcessedStyle.Alignment.Horizontal) of
      ...
      ZHJustify:
        s := 'justify';
      ZHCenterAcrossSelection:
        s := 'center';
      ZHDistributed:
        s := 'center';
      ZHJustifyDistributed:
        s := 'justify';
    end;
...
begin
  ...
  try
    ...
    ZEWriteHeaderCommon(_xml, 'UTF-8', '');
...
var
  ...
  begin
    ...
    b := (_SheetOptions.SplitHorizontalMode = ZSplitSplit) or (_SheetOptions.SplitHorizontalMode = ZSplitSplit);
...
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if (CellData[i] <> AnsiChar(#13)) then
            s := s + CellData[i];
        end;
        if (s > '') then
          xml.WriteTag(ZETag_text_p, s, true, false, true);
  ...
  begin
    ...
    begin
      ...
      if DivedIntoHeader and (i = ProcessedSheet.ColsToRepeat.Till) then
      begin
    ...
    begin
      ...
      begin
        ...
        else if (NumTopLeft >= 0) then
        begin
          t := ProcessedSheet.MergeCells.Items[NumTopLeft].Right - ProcessedSheet.MergeCells.Items[NumTopLeft].Left + 1;
          _xml.Attributes.Add('table:number-columns-spanned', IntToStr(t), false);
          t := ProcessedSheet.MergeCells.Items[NumTopLeft].Bottom - ProcessedSheet.MergeCells.Items[NumTopLeft].Top + 1;
          ...
          ZENumber:
            begin
              ...
              if (t and ZE_NUMFORMAT_NUM_IS_PERCENTAGE = ZE_NUMFORMAT_NUM_IS_PERCENTAGE) then
                ss := 'percentage'
              else if (t and ZE_NUMFORMAT_NUM_IS_CURRENCY = ZE_NUMFORMAT_NUM_IS_CURRENCY) then
              begin
        ...
        if (ss > '') then
        begin
          if not (SameText(Copy(ss, 1, 3), 'of:') or SameText(Copy(ss, 1, 5), 'oooc:')) then
          begin
            if (ss[1] <> '=') then
              ss := '=' + ss;
            tmpFormula := ZEA1ToR1C1(ss, j, i, 0, true);
            if (tmpFormula > '') then
              ss := ZER1C1ToA1(tmpFormula, j, i, ZE_RTA_ODF or ZE_RTA_ODF_PREFIX, true);
          end;
      ...
      if DivedIntoHeader and (i = ProcessedSheet.RowsToRepeat.Till) then
      begin
    ...
    begin
      ...
      s := #39 + ProcessedSheet.Title + #39 + '.' + Copy(ss, 1, pos(':', ss)) + #39 + ProcessedSheet.Title + #39 + '.' + Copy(ss, pos(':', ss) + 1, Length(ss));
...
function ODFCreateManifest(Stream: TStream): integer;
var
  _xml: TZsspXMLWriterH;
...
begin
  ...
  try
    ...
    ODFCreateStyles(XMLSS, Stream, _pages, _names, kol, _WriteHelper);
    FreeAndNil(Stream);

    Stream := TFileStream.Create(PathName + 'content.xml', fmCreate);
    ODFCreateContent(XMLSS, Stream, _pages, _names, kol, _WriteHelper);
    FreeAndNil(Stream);

    Stream := TFileStream.Create(PathName + 'meta.xml', fmCreate);
    ODFCreateMeta(XMLSS, Stream);
    FreeAndNil(Stream);

    Stream := TFileStream.Create(PathName + 'settings.xml', fmCreate);
    ODFCreateSettings(XMLSS, Stream, _pages, _names, kol);
    FreeAndNil(Stream);
...
//RETURN
//      integer
// SaveXmlssToODFSPath (no-arg overload merged into main function)
...
begin
  ...
  try
    ...
    Zipper.Entries.AddFileEntry(Stream, 'mimetype');

    Stream := TMemoryStream.Create;
    ODFCreateStyles(XMLSS, Stream, _pages, _names, kol, _WriteHelper);
    Zipper.Entries.AddFileEntry(Stream, 'styles.xml');

    Stream := TMemoryStream.Create;
    ODFCreateContent(XMLSS, Stream, _pages, _names, kol, _WriteHelper);
    Zipper.Entries.AddFileEntry(Stream, 'content.xml');

    Stream := TMemoryStream.Create;
    ODFCreateMeta(XMLSS, Stream);
    Zipper.Entries.AddFileEntry(Stream, 'meta.xml');

    Stream := TMemoryStream.Create;
    ODFCreateSettings(XMLSS, Stream, _pages, _names, kol);
    Zipper.Entries.AddFileEntry(Stream, 'settings.xml');

    Stream := TMemoryStream.Create;
    ODFCreateManifest(Stream);
    Zipper.Entries.AddFileEntry(Stream, 'META-INF/manifest.xml');

    Zipper.ZipAllFiles;
  finally
    ZESClearArrays(_pages, _names);
    if Assigned(Zipper) then
    begin
      for i := 0 to Zipper.Entries.Count - 1 do
        Zipper.Entries[i].Stream.Free;
      FreeAndNil(Zipper);
    end;
    if Assigned(_WriteHelper) then
      FreeAndNil(_WriteHelper);
...
procedure ZEReadODFCellStyleItem(var xml: TZsspXMLReaderH; var _style: TZSTyle; var StyleProperties: TZEODFStyleProperties);
var
  t: integer;
  HAutoForced: boolean;
  r: real;
    ...
    begin
      ...
      begin
        if not TryStrToInt(s, t) // ODS 1.1 - pure integer - failed
          then
        begin // ODF 1.2+ ? float with units ?
          s := LowerCase(Trim(s));
        ...
        if s > '' then
        begin  // need reduce to -180 to +180
          t := t mod 360;
          if t > +180 then
            t := t - 360;
          if t < -180 then
      ...
      _style.Alignment.VerticalText := 'ttb' = xml.Attributes['style:direction'];
...
var
  ...
  begin
    ...
    begin
      ...
      ZEReadODFCellStyleItem(xml, _Style, ReadHelper.StylesProperties[num]);
    end;
  end; //_ReadOneStyle
...
var
  ...
  _RowDefaultStyleCFNumber: integer;
  _CellStyleCFNumber: integer;
  ...
  function _FindStyleID(const st: string; out retCFIndex: integer): integer;
  var
    i: integer;
  begin
    result := -1;
  ...
  var
    ...
    begin
      ...
      if (xml.TagType = 4) then
        ZEReadODFCellStyleItem(xml, _style, ODFStyles[StyleCount]);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          else if (_stylefamily = 'table-row') then
            _ReadTableRowStyle()
          else if (_stylefamily = 'table') then
            _ReadTableStyle()
          else if (_stylefamily = 'table-cell') then
            _ReadCellStyle();
        end
        else
          ReadHelper.NumberStylesHelper.ReadKnownNumberFormat(xml);
  ...
  var
    ...
    begin
      ...
      begin
        ...
        if (s > '') then
          _CurrCell.CellStyle := _FindStyleID(s, _CellStyleCFNumber)
        else
        begin
        ...
        s := xml.Attributes.ItemsByName['table:protected'];
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (_isnf) then
                _celltext := _celltext + sLineBreak;
              ...
              begin
                ...
                if ((xml.TagName <> ZETag_text_p) and (xml.TagName <> 'text:a') and (xml.TagName <> 'text:s') and (xml.TagName <> 'text:span')) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Length(s) > 0) then
        begin
          _RowDefaultStyleID := _FindStyleID(s, _RowDefaultStyleCFNumber);
    ...
    ReadHelper.ConditionReader.ApplyConditionStylesToSheet(_CurrentPage, ReadHelper.StylesProperties, ReadHelper.StylesCount, ODFStyles, StyleCount);
  ...
  begin
    ...
    begin
      ...
      if (IfTag('table:database-range', 5)) then
        _ReadAutoFilter();
    end;
  end; //_ReadDocument
...
begin
  ...
  try
    ...
    begin
      ...
      else if (xml.TagType = 4) and (xml.TagName = 'config:config-item') then
        _ReadConfigItem();
    end; //while
...
// odszipfuncimpl.inc merged inline
