...
/// </remarks>
unit Ths.Erp.Utils.PreviewHandler;
...
type
 ...
 /// </summary>
  ...
  TPreviewHandler = class(TComponent)
  private
  ...
  public
    ...
    /// </param>
    constructor Create(aOwner: TComponent; aControl: TWinControl);reintroduce; overload;
...
constructor TPreviewHandler.Create(aOwner: TComponent; aControl: TWinControl);
begin
  inherited Create(aOwner);
  Control := aControl;
end;

destructor TPreviewHandler.Destroy;
begin
...
procedure TPreviewHandler.Bosalt;
begin
  if Assigned(FHandler) then
    FHandler.Unload;
end;

procedure TPreviewHandler._DelRef;
begin
  if Assigned(FHandler) then
    FHandler._Release;
end;

function TPreviewHandler.GetClassID: String;
var
  aRegistry: TRegistry;
  aExtens: String;
  aRegPath:  String;
...
function TPreviewHandler.GetFileExt: String;
begin
...
procedure TPreviewHandler.PreviewFile;
var
  aGUID: TGUID;
  aRect: TRect;
...
{ Bunu daha çok word ve excel kullanıyor... }
  if (FHandler.QueryInterface(IInitializeWithFile, FileInit) = 0) then
  begin
...

{
  ...
  try
    ...
    if FHandler.DoPreview = S_OK then
      Mesaj := ''; // 'The operation completed successfully.';
    if FHandler.DoPreview = E_PREVIEWHANDLER_DRM_FAIL then
      Mesaj := 'Telif hakları gereği bu dosya dijital haklar yönetimi tarafından engellendi.'; //'Blocked by digital rights management.';
    if FHandler.DoPreview = E_PREVIEWHANDLER_NOAUTH then
      Mesaj := 'Dosya izinleri engellendi.'; // 'Blocked by file permissions.';
    if FHandler.DoPreview = E_PREVIEWHANDLER_NOTFOUND then
      Mesaj := 'Nesne bulunamadı.'; // 'Item was not found.';
    if FHandler.DoPreview = E_PREVIEWHANDLER_CORRUPT then
      Mesaj := 'Önizleme yapan program düzgün yüklenmemiş veya ilgili fonksiyonu bozuk.' + sLineBreak
             + 'Bu noktada ağ yöneticinizden yardım talep edebilirsiniz; Ağ yöneticiniz şunları yapmalıdır;' + sLineBreak + sLineBreak
             + ' 1) İlgili programı bilgisayarınızdan kaldırın (uninstall)' + sLineBreak
             + ' 2) Kaldırma işlemi bittikten sonra Registry kayıtlarını temizleyen programlardan biriyle (CCleaner gibi) temizlik yapın' + sLineBreak
             + ' 3) Kaldırdığınız programın eskiden kurulu olduğu Klasörleri silin (Genelde Program Files klasörünün altında olurlar)' + sLineBreak
             + ' 4) 2. adımı tekrar yapın' + sLineBreak
             + ' 5) Bilgisayarınızı yeniden başlatın' + sLineBreak
             + ' 6) Kaldırdığınız programı sağlıklı bir şekilde yeniden kurun' + sLineBreak
             ; // 'Item was corrupt.';
    ...
    if (FControl.Visible = TRUE) then begin
      ...
      begin
        try
          ...
          if pos('streaminit', FKip) > 0 then
            FHandler._AddRef; // Şart ! .... Referans sayımını tetiklemek ve yeni bir dosyanın yüklendiğini IPreviewHandler arabirimine bildirmek için bu şart...
        except
          on E: Exception do
...
procedure TPreviewHandler.Preview;
begin
  PreviewFile;
end;
 
procedure TPreviewHandler.SetFileName(const Value: string);
begin
