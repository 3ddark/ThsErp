...
type
  TfrmBaseDBGrid2 = class(TForm)
    ...
    procedure btnSpinDownClick(Sender: TObject);
...
//    procedure EventAlerterAlert(ASender: TFDCustomEventAlerter; const AEventName: string; const AArgument: Variant);
  public
    property Table: TTableBase read FTable write SetTable;
    ...
    property TableHelper: TTableBase read FTableHelper write FTableHelper;
    //for use HelperForm
  ...
  published
    ...
    procedure stbBaseDrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel; const Rect: TRect);
    procedure WmAfterShow(var Msg: TMessage); message WM_AFTER_SHOW;
    procedure btnAcceptClick(Sender: TObject);

    procedure WriteRecordCount(ACount: Integer);
...
var
  frmBaseDBGrid2: TfrmBaseDBGrid2;
...
procedure TfrmBaseDBGrid2.actfilterExecute(Sender: TObject);
var
...
procedure TfrmBaseDBGrid2.actfilter_backExecute(Sender: TObject);
begin
  if FiltreGrid.Count > 0 then
  begin
...
procedure TfrmBaseDBGrid2.actfilter_excludeExecute(Sender: TObject);
var
  LFilterBefore, LFilterVal: string;
begin
  LFilterBefore := '';
...
procedure TfrmBaseDBGrid2.actfilter_initExecute(Sender: TObject);
begin
...
procedure TfrmBaseDBGrid2.actfilter_removeExecute(Sender: TObject);
begin
  FFiltreGrid.Clear;
...
procedure TfrmBaseDBGrid2.actsortExecute(Sender: TObject);
var
  sl: TStringList;
  LOrderList: string;
...
procedure TfrmBaseDBGrid2.actsort_initExecute(Sender: TObject);
begin
  mnisort_remove.Visible := False;
end;

procedure TfrmBaseDBGrid2.actsort_removeExecute(Sender: TObject);
begin
  TZQuery(grd.DataSource.DataSet).IndexFieldNames := '';
  mnisort_remove.Visible := False;
end;

procedure TfrmBaseDBGrid2.btnAcceptClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmBaseDBGrid2.btnAddNewClick(Sender: TObject);
begin
  ShowInputForm(Sender, ifmNewRecord);
end;

procedure TfrmBaseDBGrid2.btnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmBaseDBGrid2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if FIsHelper then
  begin
    if DataAktar then
...
procedure TfrmBaseDBGrid2.FormCreate(Sender: TObject);
begin
  ...
  qrybase.Connection := TTableManager<TObject>(Table.Manager).Connection;
  dsbase.DataSet := qrybase;
...
procedure TfrmBaseDBGrid2.FormDestroy(Sender: TObject);
begin
  if FIsHelper then
  begin
...
procedure TfrmBaseDBGrid2.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  begin
...
procedure TfrmBaseDBGrid2.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = Char(VK_ESCAPE)) then //Escape (Close dont do any process)
  begin
    Key := #0;
    if FIsHelper then
      FDataAktar := False;
...
procedure TfrmBaseDBGrid2.FormKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if FIsHelper then
  begin
    if Key = VK_F5 then
...
procedure TfrmBaseDBGrid2.FormPaint(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.btnSpinDownClick(Sender: TObject);
begin
  MoveDown();
end;

procedure TfrmBaseDBGrid2.btnSpinUpClick(Sender: TObject);
begin
  MoveUp();
end;
...
function TfrmBaseDBGrid2.CreateInputForm(Sender: TObject; pFormMode: TInputFormMode): TForm;
begin
  Result := nil;
end;

procedure TfrmBaseDBGrid2.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  WriteRecordCount(grd.DataSource.DataSet.RecordCount);
end;

procedure TfrmBaseDBGrid2.grdCellClick(Column: TColumn);
begin
  SetSelectedItem;
end;

procedure TfrmBaseDBGrid2.grdColEnter(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdColExit(Sender: TObject);
begin
  inherited;
  grd.Repaint;
end;

procedure TfrmBaseDBGrid2.grdColumnMoved(Sender: TObject; FromIndex, ToIndex: Integer);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdDblClick(Sender: TObject);
begin
  if FIsHelper then
    SelectCurrentRecord
  else
...
procedure TfrmBaseDBGrid2.grdDragDrop(Sender, Source: TObject; X, Y: Integer);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  inherited;
  //
end;
...
begin
  //Satırı renklendir.
  LColorActive := GSysUygulamaAyari.GridColorActive.AsInteger;
  LColor1 := GSysUygulamaAyari.GridColor1.AsInteger;
  LColor2 := GSysUygulamaAyari.GridColor2.AsInteger;
...
procedure TfrmBaseDBGrid2.grdDrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdEnter(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdExit(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmBaseDBGrid2.grdKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.grdMouseLeave(Sender: TObject);
begin
  inherited;
//  grd.Repaint;
end;

procedure TfrmBaseDBGrid2.grdMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
//  grd.Repaint;
end;

procedure TfrmBaseDBGrid2.grdMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  inherited;
//  grd.Repaint;
end;

procedure TfrmBaseDBGrid2.grdMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint; var Handled: Boolean);
begin
  inherited;
//
end;

procedure TfrmBaseDBGrid2.grdMouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  inherited;
//
end;

procedure TfrmBaseDBGrid2.grdMouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  inherited;
//
end;

procedure TfrmBaseDBGrid2.grdTitleClick(Column: TColumn);
begin
  //
  actsortExecute(Column);
end;

procedure TfrmBaseDBGrid2.drawTriangleInRect(ARect: TRect; ASort: TSortType; AAlignment: TAlignment);
const
  OFFSET = 2;
var
  goLeft: integer;
...
procedure TfrmBaseDBGrid2.edtFilterHelperKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  //helper için aşağı yukarıda key up eventte sadece grid hareket etsin.
  //aşağı yukarı yönler edit içinde selstart position değişimi yapmasın
...
procedure TfrmBaseDBGrid2.FormResize(Sender: TObject);
begin
  inherited;
  //
end;

procedure TfrmBaseDBGrid2.FormShow(Sender: TObject);
var
  n1: Integer;
begin
  ...
  if TTableManager<TObject>(Table.Manager).Authorized(Ths.Erp.Database.TableManager.ptAddRecord, True) then
  begin
    btnAddNew.Visible := True;
  ...
  if GSysKullanici.IsSuperUser.Value then
  begin
...
//    mniColumnSummary.Visible := True;
  ...
  begin
    ...
    pgalertbase.Connection := TTableManager<TObject>(Table.Manager).Connection;
    pgalertbase.Events.Add(TTableManager<TObject>(Table.Manager).TableName);
...
function TfrmBaseDBGrid2.GetFieldByFieldName(pFieldName: string; pGridColumns: TDBGridColumns): TField;
var
  nIndex: Integer;
begin
  Result := nil;
...
function TfrmBaseDBGrid2.getFilterEditData: string;
var
  LField: TField;
begin
  Result := '';
...
function TfrmBaseDBGrid2.GetLowHighEqual(pField: TField; pDefaultColor: TColor): Integer;
var
  n1: Integer;
begin
...
function TfrmBaseDBGrid2.GetPercentMaxVal(pField: TField): Double;
var
  n1: Integer;
begin
  Result := 1;
...
function TfrmBaseDBGrid2.IsRenkliRakamVar(pFieldName: string): Boolean;
var
  n1: Integer;
begin
  Result := False;
...
function TfrmBaseDBGrid2.IsYuzdeCizimAlaniVar(pFieldName: string): Boolean;
var
  n1: Integer;
begin
  Result := False;
...
procedure TfrmBaseDBGrid2.mnicopy_recordClick(Sender: TObject);
begin
...
procedure TfrmBaseDBGrid2.mnicopy_to_clipboardClick(Sender: TObject);
begin
  if grd.Focused then
  begin
...
procedure TfrmBaseDBGrid2.mniColumnSummaryClick(Sender: TObject);
var
  LSummary: TSysGridColumn;
  LColumn: TColumn;
begin
  ...
  begin
    ...
    begin
      ...
      LSummary.ColumnName.Value := ReplaceRealColOrTableNameTo(LColumn.FieldName);
...
procedure TfrmBaseDBGrid2.mniColumnTitleByLangClick(Sender: TObject);
var
...
begin
  ...
  begin
    vSysLangGuiContent := TSysLangGuiContent.Create(GDataBase);

    vSysLangGuiContent.Lang.Value := GDataBase.ConnSetting.Language;
    vSysLangGuiContent.Code.Value := AColumn.FieldName;
    vSysLangGuiContent.ContentType.Value := LngDGridFieldCaption;
    vSysLangGuiContent.TablaNameFld.Value := TTableManager<TObject>(Table.Manager).TableName;
    vSysLangGuiContent.Value.Value := AColumn.Title.Caption;
...
procedure TfrmBaseDBGrid2.mniLangDataContentClick(Sender: TObject);
var
...
begin
  ...
  begin
    vSysLangDataContent := TSysLangDataContent.Create(GDataBase);

    vSysLangDataContent.Lang.Value := GDataBase.ConnSetting.Language;
...
procedure TfrmBaseDBGrid2.mniexport_excel_allClick(Sender: TObject);
begin
  TransferToExcel(True);
  //TransferToExcelAll();
end;

procedure TfrmBaseDBGrid2.mniexport_excelClick(Sender: TObject);
begin
  TransferToExcel();
end;

procedure TfrmBaseDBGrid2.mniFormTitleByLangClick(Sender: TObject);
begin
  //CreateLangGuiContentFormforFormCaption;
end;

procedure TfrmBaseDBGrid2.mnipreviewClick(Sender: TObject);
begin
...
procedure TfrmBaseDBGrid2.mnisort_removeClick(Sender: TObject);
begin
...
procedure TfrmBaseDBGrid2.MoveDown;
begin
  qrybase.DataSource.DataSet.Prior;
  SetSelectedItem();
end;

procedure TfrmBaseDBGrid2.MoveUp;
begin
  qrybase.DataSource.DataSet.Next;
  SetSelectedItem();
end;

procedure TfrmBaseDBGrid2.pgalertbaseNotify(Sender: TObject; Event: string; ProcessID: Integer; Payload: string);
begin
  inherited;
...
procedure TfrmBaseDBGrid2.pmDBChange(Sender: TObject; Source: TMenuItem; Rebuild: Boolean);
begin
//
end;

procedure TfrmBaseDBGrid2.pmDBPopup(Sender: TObject);
begin
  SetSelectedItem();
end;

procedure TfrmBaseDBGrid2.RefreshData;
begin
...
procedure TfrmBaseDBGrid2.RefreshDataFirst();
var
  AField: TField;
  AFieldDB: TFieldDB;
  ACol: TColumn;
...
//  AGridCols := TObjectList<TSysGridColumn>.Create(False);
  try
    dsbase.OnDataChange := DataSourceDataChange;
    ...
    qrybase.SQL.Text := TTableManager<TObject>(Table.Manager).GetDsSQL(FQryFiltreVarsayilan + FQryFiltreVarsayilanKullanici, FQrySiralamaVarsayilan);
    qrybase.Open;
{
    //sayısal bilgilerde otomatik formatlama ve kolonların çıkma sırasını ayarla işlemini yap
    vHaneSayisi := 2;
    if FormOndalikMod = fomBuying then
      vHaneSayisi := FormatedVariantVal(GSysOndalikHane.BuyingPrice)
    else if FormOndalikMod = fomSale  then
      vHaneSayisi := FormatedVariantVal(GSysOndalikHane.SalesPrice)
    else if FormOndalikMod = fomStock then
      vHaneSayisi := FormatedVariantVal(GSysOndalikHane.StockQuantity);


    for n2 := 0 to qrybase.FieldCount-1 do
    begin
      if not ExistsGridColumn(qrybase.Fields.Fields[n2].FieldName) then
      begin  AFieldDB := Table.GetFieldByFieldName(qrybase.Fields.Fields[n2].FieldName);
        if AFieldDB <> nil then
          AddColumn(qrybase.Fields.Fields[n2].FieldName, AFieldDB.Title);
      end;
    end;
    ...
    for n1 := 0 to GGridColWidth.List.Count-1 do
      if TSysGridColumn(GGridColWidth.List[n1]).TableNameFld.AsString = LTableName then
        AGridCols.Add(TSysGridColumn(GGridColWidth.List[n1]));
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if  (TSysGridColumn(AGridCols[n1]).TableNameFld.AsString = LTableName)
          and (AField.FieldName = ReplaceToRealColOrTableName(VarToStr(FormatedVariantVal(TSysGridColumn(AGridCols[n1]).ColumnName))))
          then
          begin
            ...
            if AFieldDB <> nil then
              SetDisplayFormat(AFieldDB, AField, TSysGridColumn(AGridCols[n1]));

            if Assigned(ACol) then
            begin
              with TSysGridColumn(AGridCols[n1]) do
              try
                ACol.Index := VarToInt(FormatedVariantVal(TSysGridColumn(AGridCols[n1]).SeqNo));
                ACol.Width := VarToInt(FormatedVariantVal(TSysGridColumn(AGridCols[n1]).ColWidth));
                ACol.Visible := FormatedVariantVal(TSysGridColumn(AGridCols[n1]).IsShow);
              except
                on E: Exception do
                begin
      ...
      for n3 := 0 to Length(FColoredPercentColNames)-1 do
        for n1 := 0 to AGridCols.Count-1 do
          for n2 := 0 to grd.Columns.Count-1 do
          begin
            LColName := TSysGridColumn(AGridCols[n1]).ColumnName.AsString;
            if  (TSysGridColumn(AGridCols[n1]).TableNameFld.AsString = LTableName)
            and (ReplaceToRealColOrTableName(LColName) = grd.Columns.Items[n2].FieldName)
            then
            begin
              LColPercent.FieldName := LColName;
              LColPercent.MaxValue := TSysGridColumn(AGridCols[n1]).MaxValuePercent.AsFloat;
              LColPercent.ColorBar := TSysGridColumn(AGridCols[n1]).ColorBar.AsInteger;
              LColPercent.ColorBarBack := TSysGridColumn(AGridCols[n1]).ColorBarBack.AsInteger;
              LColPercent.ColorBarText := TSysGridColumn(AGridCols[n1]).ColorBarText.AsInteger;
      ...
      for n1 := 0 to AGridCols.Count-1 do
      begin
        if (TSysGridColumn(AGridCols[n1]).TableNameFld.AsString = LTableName) then
        begin
          LColName := TSysGridColumn(AGridCols[n1]).ColumnName.AsString;
          LColColor.FieldName := ReplaceToRealColOrTableName(LColName);
          LColColor.MinValue := TSysGridColumn(AGridCols[n1]).MinValue.AsInteger;
          LColColor.MinColor := TSysGridColumn(AGridCols[n1]).MinColor.AsInteger;
          LColColor.MaxValue := TSysGridColumn(AGridCols[n1]).MaxValue.AsInteger;
          LColColor.MaxColor := TSysGridColumn(AGridCols[n1]).MaxColor.AsInteger;
...
procedure TfrmBaseDBGrid2.RefreshGrid;
begin
  WriteRecordCount(grd.DataSource.DataSet.RecordCount);
end;

procedure TfrmBaseDBGrid2.mnifilter_removeClick(Sender: TObject);
begin
...
procedure TfrmBaseDBGrid2.SelectCurrentRecord;
begin
  FDataAktar := True;
  if grd.DataSource.DataSet.RecordCount > 0 then
  begin
    SetSelectedItem;
    TTableManager<TObject>(Table.Manager).GetByID(Table.Id.Value, False, False);
    TableHelper := Table.Clone;
  end;
  btnCloseClick(btnClose);
end;

procedure TfrmBaseDBGrid2.SetColVisible(pFieldName: string; pVisible: Boolean);
var
  n1: Integer;
begin
  for n1 := 0 to grd.Columns.Count-1 do
...
procedure TfrmBaseDBGrid2.SetParentForm(const Value: TForm);
begin
  FParentForm := Value;
end;

procedure TfrmBaseDBGrid2.SetSelectedItem;
var
  AField: TField;
  n1, n2: Integer;
begin
...
procedure TfrmBaseDBGrid2.SetTable(const Value: TTableBase);
begin
  FTable := Value;
end;

procedure TfrmBaseDBGrid2.SetTitleFromLangContent;
var
  n1, n2: Integer;
...
procedure TfrmBaseDBGrid2.ShowInputForm(Sender: TObject; pFormType: TInputFormMode);
var
  LForm: TForm;
begin
...
begin
  ...
  stbBase.Panels.Items[STATUS_USERNAME].Text := TranslateText('Dönem', FrameworkLang.GeneralPeriod, LngGeneral, LngSystem) + ' ' + GSysUygulamaAyari.Period.AsString;

  if GDataBase.Connection.Connected then
    stbBase.Panels.Items[STATUS_KEY_F4].Text := GSysUygulamaAyari.Title.AsString;
end;

procedure TfrmBaseDBGrid2.stbBaseDrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel; const Rect: TRect);
var
  vIco: Integer;
begin
...
procedure TfrmBaseDBGrid2.Timer1Timer(Sender: TObject);
begin
  //
end;

procedure TfrmBaseDBGrid2.TransferToExcel(AAllColumn: Boolean = False);
var
...
begin
  ...
  dlgSave.FileName := Self.Caption + ' ' + DateToStr(GDatabase.GetToday);
...
procedure TfrmBaseDBGrid2.TransferToExcelAll();
//var
//  vFileName: string;
var
//  nR: Integer;
...
//  OpenWithControl := isCtrlDown;
  ...
  if (qrybase.Filter <> '') then
    if (UpperCase(Trim(LeftStr(qrybase.Filter, 5))) = 'AND') then
      AGridFilter := qrybase.Filter + AGridFilter
    else
      AGridFilter := ' AND ' + qrybase.Filter + AGridFilter;
{
  ATable := Table.Clone;
...
procedure TfrmBaseDBGrid2.WmAfterShow(var Msg: TMessage);
//var
//  n1: Integer;
begin
  if FIsHelper then
...
procedure TfrmBaseDBGrid2.WriteRecordCount(ACount: Integer);
begin
  if (GDataBase.Connection.Connected) and (stbBase.Panels.Count > 0) then
    stbBase.Panels.Items[STATUS_SQL_SERVER].Text := TranslateText('Total', FrameworkLang.GeneralRecordCount, LngGeneral, LngSystem) + ': ' + ACount.ToString;
end;
