...
uses
  ...
  Ths.Database.Table.ChHesapPlanlari, ufrmAccHesapPlanlari, Ths.Constants;

type
  TfrmAccHesapKartiAra = class(TfrmBaseInputDB)
...
procedure TfrmAccHesapKartiAra.cbbara_hesap_koduExit(Sender: TObject);
begin
  inherited;
  edthesap_kodu.Text := getHesapKodu;
end;

procedure TfrmAccHesapKartiAra.edtkok_hesap_koduExit(Sender: TObject);
begin
  inherited;
  edthesap_kodu.Text := getHesapKodu;
end;

procedure TfrmAccHesapKartiAra.fillAraHesapNumbers;
var
  n1: Integer;
...
procedure TfrmAccHesapKartiAra.FormShow(Sender: TObject);
begin
...
function TfrmAccHesapKartiAra.getHesapKodu: string;
begin
  Result := edtkok_hesap_kodu.Text + '-' + cbbara_hesap_kodu.Text
end;

procedure TfrmAccHesapKartiAra.HelperProcess(Sender: TObject);
var
  LFrmHesapPlani: TfrmAccHesapPlanlari;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LFrmHesapPlani := TfrmAccHesapPlanlari.Create(TEdit(Sender), Self, LHesapPlani, fomNormal, True);
        try
...
procedure TfrmAccHesapKartiAra.RefreshData();
var
...
procedure TfrmAccHesapKartiAra.Repaint;
begin
  inherited;
  edthesap_kodu.ReadOnly := True;
end;

procedure TfrmAccHesapKartiAra.btnAcceptClick(Sender: TObject);
begin
  if (FormMode = ifmNewRecord) or (FormMode = ifmCopyNewRecord) or (FormMode = ifmUpdate) then
  begin
