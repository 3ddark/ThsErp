...
type
  TfrmSysGuiContents = class(TfrmBaseDBGrid)
  protected
...
uses Ths.Constants, ufrmSysGuiContent, Ths.Database.Table.SysGuiContent;

{$R *.dfm}

function TfrmSysGuiContents.CreateInputForm(Sender: TObject; pFormMode: TInputFormMode): TForm;
begin
  Result := nil;
  if (pFormMode = ifmRewiev) then
    Result := TfrmSysGuiContent.Create(Self, Self, Table.Clone(), pFormMode)
  else if (pFormMode = ifmNewRecord) then
    Result := TfrmSysGuiContent.Create(Self, Self, TSysGridContents.Create(Table.Database), pFormMode)
