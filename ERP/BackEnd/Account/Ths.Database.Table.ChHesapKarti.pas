...
begin
  ...
  FHesapKodu := TFieldDB.Create('hesap_kodu', ftString, '', Self);
  FHesapIsmi := TFieldDB.Create('hesap_ismi', ftString, '', Self);
  FHesapTipiID := TFieldDB.Create('hesap_tipi_id', ftInteger, 0, Self);
  FHesapTipi := TFieldDB.Create(FSetChHesapTipi.HesapTipi.FieldName, FSetChHesapTipi.HesapTipi.DataType, FSetChHesapTipi.HesapTipi.Value, Self);
  FGrupID := TFieldDB.Create('grup_id', ftInteger, 0, Self);
  FGrup := TFieldDB.Create(FChGrup.Grup.FieldName, FChGrup.Grup.DataType, FChGrup.Grup.Value, Self);
  FBolgeID := TFieldDB.Create('bolge_id', ftInteger, 0, Self);
  FBolge := TFieldDB.Create(FChBolge.Bolge.FieldName, FChBolge.Bolge.DataType, FChBolge.Bolge.Value, Self);
  FMukellefTipi := TFieldDB.Create('mukellef_tipi', ftSmallint, 0, Self);
  FMukellefAdi := TFieldDB.Create('mukellef_adi', ftString, '', Self);
  FMukellefAdi2 := TFieldDB.Create('mukellef_adi2', ftString, '', Self);
  ...
  FIban := TFieldDB.Create('iban', ftString, '', Self);
  FIbanPara := TFieldDB.Create('iban_para', ftString, '', Self);
  FNace := TFieldDB.Create('nace', ftString, '', Self);
  ...
  FYetkili3 := TFieldDB.Create('yetkili3', ftString, '', Self);
  FYetkili3Tel := TFieldDB.Create('yetkili3_tel', ftString, '', Self);
  FFaks := TFieldDB.Create('faks', ftString, '', Self);
  FMuhasebeTelefon := TFieldDB.Create('muhasebe_telefon', ftString, '', Self);
  FMuhasebeEmail := TFieldDB.Create('muhasebe_email', ftString, '', Self);
  FMuhasebeYetkili := TFieldDB.Create('muhasebe_yetkili', ftString, '', Self);
  FOzelNot := TFieldDB.Create('ozel_not', ftString, '', Self);
  FKokKod := TFieldDB.Create('kok_kod', ftString, '', Self);
  FAraKod := TFieldDB.Create('ara_kod', ftString, '', Self);
  FIskonto := TFieldDB.Create('iskonto', ftFloat, 0, Self);
  FEFaturaKullaniyor := TFieldDB.Create('efatura_kullaniyor', ftBoolean, False, Self);
  FEFaturaPBName := TFieldDB.Create('efatura_pb_name', ftString, '', Self);
  FAdresID := TFieldDB.Create('adres_id', ftInteger, 0, Self);
  FPasif := TFieldDB.Create('pasif', ftBoolean, False, Self);
  FSeviyeSayisi := TFieldDB.Create(FChHesapPlani.Seviye.FieldName, FChHesapPlani.Seviye.DataType, FChHesapPlani.Seviye.Value, Self);
