...
//    property DataType: TDataType read FDataType;
  ...
  /// <summary>
  ///   Birden fazla kolonu birincil anahtar olarak tanımlar.
  ///   Kullanım: [CompositeKey('col1,col2,col3')]
  ///   Repository, bu attribute'u öncelikli olarak okur.
  /// </summary>
  CompositeKey = class(TCustomAttribute)
  private
    FColumns: string;
    function GetColumnArray: TArray<string>;
  public
    constructor Create(const AColumns: string);
    /// <summary>Virgülle ayrılmış kolon adları: 'col1,col2'</summary>
    property Columns: string read FColumns;
    /// <summary>Kolon adlarını dizi olarak döndürür (boşluk temizlenir)</summary>
    property ColumnArray: TArray<string> read GetColumnArray;
...
constructor CompositeKey.Create(const AColumns: string);
begin
  inherited Create;
  FColumns := AColumns;
end;

function CompositeKey.GetColumnArray: TArray<string>;
var
  Parts: TArray<string>;
  i: Integer;
begin
  Parts := FColumns.Split([',']);
  SetLength(Result, Length(Parts));
  for i := 0 to High(Parts) do
    Result[i] := Trim(Parts[i]);
end;

function CompositeKey.ColumnCount: Integer;
begin
  Result := Length(GetColumnArray);
end;
