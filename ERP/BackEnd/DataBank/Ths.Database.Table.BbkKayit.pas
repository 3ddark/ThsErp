...
begin
  ...
  FFirmaAdi := TFieldDB.Create('firma_adi', ftWideString, '', Self);
  FTel1 := TFieldDB.Create('tel1', ftWideString, '', Self);
  FTel2 := TFieldDB.Create('tel2', ftWideString, '', Self);
  FTel3 := TFieldDB.Create('tel3', ftWideString, '', Self);
  FFax := TFieldDB.Create('fax', ftWideString, '', Self);
  FEmail := TFieldDB.Create('email', ftWideString, '', Self);
  FWeb := TFieldDB.Create('web', ftWideString, '', Self);
  FYetkili1 := TFieldDB.Create('yetkili1', ftWideString, '', Self);
  FYetkili1Tel := TFieldDB.Create('yetkili1_tel', ftWideString, '', Self);
  FYetkili1Email := TFieldDB.Create('yetkili1_email', ftWideString, '', Self);
  FYetkili2 := TFieldDB.Create('yetkili2', ftWideString, '', Self);
  FYetkili2Tel := TFieldDB.Create('yetkili2_tel', ftWideString, '', Self);
  FYetkili2Email := TFieldDB.Create('yetkili2_email', ftWideString, '', Self);
  FAdres := TFieldDB.Create('adres', ftWideString, '', Self);
  ...
  FUlkeID := TFieldDB.Create('ulke_id', FSysSehir.UlkeID.DataType, 0, Self);
  FUlke := TFieldDB.Create(FSysSehir.UlkeAdi.FieldName, FSysSehir.UlkeAdi.DataType, '', Self);
  FBolgeID := TFieldDB.Create('bolge_id', FSysSehir.BolgeID.DataType, 0, Self);
  FBolge := TFieldDB.Create(FSysSehir.Bolge.FieldName, FSysSehir.Bolge.DataType, '', Self);
  FNot1 := TFieldDB.Create('not1', ftWideString, '', Self);
  FNot2 := TFieldDB.Create('not2', ftWideString, '', Self);
  FFirmaTipiID := TFieldDB.Create('firma_tipi_id', ftInteger, 0, Self);
  FFirmaTipi := TFieldDB.Create(FBbkFirmaTipi.FirmaTipi.FieldName, FBbkFirmaTipi.FirmaTipi.DataType, '', Self);
  FVergiDairesi := TFieldDB.Create('vergi_dairesi', ftWideString, '', Self);
  FVergiNumarasi := TFieldDB.Create('vergi_numarasi', ftWideString, '', Self);
  ...
  FCalismaDurumu := TFieldDB.Create(FBbkCalismaDurumu.CalismaDurumu.FieldName, FBbkCalismaDurumu.CalismaDurumu.DataType, '', Self);
  FFinansDurumuID := TFieldDB.Create('finans_durumu_id', ftInteger, 0, Self);
  FFinansDurumu := TFieldDB.Create(FBbkFinansDurumu.FinansDurumu.FieldName, FBbkFinansDurumu.FinansDurumu.DataType, '', Self);
  FSevkiyatYetkilisi := TFieldDB.Create('sevkiyat_yetkilisi', ftWideString, '', Self);
  FSevkiyatYetkilisiTel := TFieldDB.Create('sevkiyat_yetkilisi_tel', ftWideString, '', Self);
end;
...
function TBbkKayit.SelectToDatasource(AFilter: string; APermissionControl: Boolean; AAllColumn: Boolean; AHelper: Boolean): string;
var
  LQry: TFDQuery;
begin
