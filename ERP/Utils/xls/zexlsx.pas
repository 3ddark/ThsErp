...
uses
  ...
  Winapi.Windows, zeformula, zsspxml, zexmlss, zesavecommon, zeZipper;
...
type
  ...
  public
    ...
    function CreateSheetRels(const Stream: TStream): integer;
...
function SaveXmlssToXLSX(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;

function SaveXmlssToXLSX(var XMLSS: TZEXMLSS; FileName: string): integer; overload;

function ExportXmlssToXLSX(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string; AllowUnzippedFolder: boolean = false): integer;
...
function ZEXLSXCreateStyles(var XMLSS: TZEXMLSS; Stream: TStream): integer;

function ZEXLSXCreateWorkBook(var XMLSS: TZEXMLSS; Stream: TStream; const _pages: TIntegerDynArray; const _names: TStringDynArray; PageCount: integer): integer;

function ZEXLSXCreateSheet(var XMLSS: TZEXMLSS; Stream: TStream; SheetNum: integer; const WriteHelper: TZEXLSXWriteHelper): integer;

function ZEXLSXCreateContentTypes(var XMLSS: TZEXMLSS; Stream: TStream; PageCount: integer; CommentCount: integer; const PagesComments: TIntegerDynArray; const WriteHelper: TZEXLSXWriteHelper): integer;

function ZEXLSXCreateRelsMain(Stream: TStream): integer;

function ZEXLSXCreateSharedStrings(var XMLSS: TZEXMLSS; Stream: TStream): integer;

function ZEXLSXCreateDocPropsApp(Stream: TStream): integer;

function ZEXLSXCreateDocPropsCore(var XMLSS: TZEXMLSS; Stream: TStream): integer;

function ZEXLSXCreateDrawing(var XMLSS: TZEXMLSS; Stream: TStream; Drawing: TZEDrawing): integer;

function ZEXLSXCreateDrawingRels(var XMLSS: TZEXMLSS; Stream: TStream; Drawing: TZEDrawing): integer;
...
uses
  System.AnsiStrings, System.StrUtils, System.Math, zenumberformats, zearchhelper;
...
type
  ...
  // For stream-based decompression
...
type
  ...
  public
    ...
    property XMLSS: TZEXMLSS read FXMLSS write FXMLSS;                  // data store
    property ListName: string read FListName write FListName;           // sheet name
    property RetCode: integer read FRetCode;                            // error code
    property FileArray[num: integer]: TZXLSXFileItem read GetFileItem;  // files
    property ArchFile[num: integer]: string read GetArchFileItem;
    property ArchFilesCount: integer read FArchFilesCount;              // number of files in archive
    property FilesCount: integer read FFilesCount;                      // file count
    property FileType: integer read FFileType write FFileType;          // file type
    property FileNumber: integer read FFileNumber write FFileNumber;    // index
    property RelationsCounts[num: integer]: integer read GetRelationsCounts; // relation count
    property RelationsCount: integer read FRelationsCount;                   // total relation count
    property RelationsArray[num: integer]: TZXLSXRelationsArray read GetRelationsArray; // relations
    property SheetRelationNumber: integer read FSheetRelationNumber;
    property isNeedReadComments: boolean read FNeedReadComments;        // whether to read comments
  end;
...
function TZEXLSXWriteHelper.CreateSheetRels(const Stream: TStream): integer;
var
...
begin
  ...
  try
    ...
    for i := 0 to FHyperLinksCount - 1 do
      ZEAddRelsRelation(_xml, 'rId' + IntToStr(FHyperLinks[i].RID), FHyperLinks[i].RelType, FHyperLinks[i].Target, FHyperLinks[i].TargetMode);
...
begin
  ...
  case num of
    0:
      result := SCHEMA_DOC_REL + '/worksheet';
    1:
      result := SCHEMA_DOC_REL + '/styles';
    2:
      result := SCHEMA_DOC_REL + '/sharedStrings';
    3:
      result := SCHEMA_DOC_REL + '/officeDocument';
    4:
      result := SCHEMA_PACKAGE_REL + '/metadata/core-properties';
    5:
      result := SCHEMA_DOC_REL + '/extended-properties';
    6:
      result := SCHEMA_DOC_REL + '/hyperlink';
    7:
      result := SCHEMA_DOC_REL + '/comments';
    8:
      result := SCHEMA_DOC_REL + '/vmlDrawing';
    9:
      result := SCHEMA_DOC_REL + '/drawing';
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      else if ((xml.TagName = 'a:sysClr') and (_b) and (xml.TagType in [TAG_TYPE_START, TAG_TYPE_CLOSED])) then
      begin
        _addFillColor(xml.Attributes.ItemsByName['lastClr']);
      end
      else if ((xml.TagName = 'a:srgbClr') and (_b) and (xml.TagType in [TAG_TYPE_START, TAG_TYPE_CLOSED])) then
      begin
      ...
      begin
        ...
        begin
          ...
          if (s = 'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml') then
            ...
            t := 1
          else if (s = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml') or (s = 'application/vnd.openxmlformats-officedocument.spreadsheetml.template.main+xml') then
            t := 2
            ...
            t := 7
          else if (s = 'application/vnd.openxmlformats-officedocument.theme+xml') then
            t := 8;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        s := xml.Attributes.ItemsByName['r']; // index
        if (s <> '') then
        begin
        ...
        s := xml.Attributes.ItemsByName['s']; // style
        if (s > '') then
        ...
        if (xml.TagType = TAG_TYPE_START) then
          ...
          begin
            ...
            else if ((xml.TagName = 'f') and (xml.TagType = TAG_TYPE_END)) then
        ...
        begin
          ...
          else if (ZEIsTryStrToFloat(v, _tfloat)) then
            v := ZEDateTimeToStr(_tfloat)
          else
            _currCell.CellType := ZEString;
        end;
      ...
      else if ((xml.TagName = 'row') and (xml.TagType in [TAG_TYPE_START, TAG_TYPE_CLOSED])) then
      begin
        _currCol := 0;
      ...
      else if ((xml.TagName = 'row') and (xml.TagType = TAG_TYPE_END)) then
      begin
        inc(_currRow);
        CheckRow(_currRow + 1);
      end;
  ...
  try
    ...
    begin
      ...
      if ((xml.TagName = 'sheetData') and (xml.TagType = TAG_TYPE_START)) then
        _ReadSheetData()
      else if ((xml.TagName = 'autoFilter') and (xml.TagType = TAG_TYPE_CLOSED)) then
        _ReadAutoFilter()
      else if ((xml.TagName = 'mergeCells') and (xml.TagType = TAG_TYPE_START)) then
        _ReadMerge()
      else if ((xml.TagName = 'cols') and (xml.TagType = TAG_TYPE_START)) then
        _ReadCols()
      else if ((xml.TagName = 'pageMargins') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
      ...
      else if ((xml.TagName = 'pageSetup') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
      ...
      else if ((xml.TagName = 'printOptions') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
      ...
      else if ((xml.TagName = 'dimension') and (xml.TagType = TAG_TYPE_CLOSED)) then
        _GetDimension()
      else if ((xml.TagName = 'hyperlinks') and (xml.TagType = TAG_TYPE_START)) then
        _ReadHyperLinks()
      else if ((xml.TagName = 'sheetViews') and (xml.TagType = TAG_TYPE_START)) then
        _ReadSheetViews()
      else if ((xml.TagType = TAG_TYPE_START) and (xml.TagName = ZETag_conditionalFormatting)) then
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ((xml.TagName = 'b') and (xml.TagType = TAG_TYPE_CLOSED)) then
          FontArray[_currFont].bold := true
        else if ((xml.TagName = 'charset') and (xml.TagType = TAG_TYPE_CLOSED)) then
        begin
          if (TryStrToInt(s, t)) then
            FontArray[_currFont].charset := t;
        end
        else if ((xml.TagName = 'color') and (xml.TagType = TAG_TYPE_CLOSED)) then
        begin
          ZXLSXGetColor(FontArray[_currFont].color, FontArray[_currFont].ColorType, FontArray[_currFont].LumFactor);
        end
        else if ((xml.TagName = 'i') and (xml.TagType = TAG_TYPE_CLOSED)) then
  ...
  //      end else
        ...
        else if ((xml.TagName = 'sz') and (xml.TagType = TAG_TYPE_CLOSED)) then
        begin
          if (TryStrToInt(s, t)) then
            FontArray[_currFont].fontsize := t;
        end
        else if ((xml.TagName = 'u') and (xml.TagType = TAG_TYPE_CLOSED)) then
        begin
          if (s > '') then
            if (s <> 'none') then
              FontArray[_currFont].underline := true;
        end
        else if ((xml.TagName = 'vertAlign') and (xml.TagType = TAG_TYPE_CLOSED)) then
        begin
        end;
      end; //if
  ...
  var
    ...
    begin
      ...
      if (s > '') then
      begin
        BorderArray[_currBorder][borderNum].isEnabled := XLSXGetBorderStyle(s, BorderArray[_currBorder][borderNum].Weight, BorderArray[_currBorder][borderNum].style);
      end;
    end; //_SetCurBorder
  ...
  begin
    ...
    begin
      ...
      else if ((xml.TagName = 'patternFill') and (xml.TagType in [TAG_TYPE_START, TAG_TYPE_CLOSED])) then
      begin
        if (_currFill >= 0) then
        begin
      ...
      else if ((xml.TagName = 'bgColor') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
        if (_currFill >= 0) then
        begin
          ZXLSXGetColor(FillArray[_currFill].patterncolor, FillArray[_currFill].patternColorType, FillArray[_currFill].lumFactorPattern);

          ZEXLSXSwapPatternFillColors(FillArray[_currFill]);
        end;
      end
      else if ((xml.TagName = 'fgColor') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
        if (_currFill >= 0) then
          ZXLSXGetColor(FillArray[_currFill].bgcolor, FillArray[_currFill].bgColorType, FillArray[_currFill].lumFactorBG);
      end; //fgColor
    end; //while
  end; //_ReadFills
  ...
  begin
    ...
    begin
      ...
      else if ((xml.TagName = 'alignment') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
        if (_currCell >= 0) then
        begin
      ...
      else if ((xml.TagName = 'protection') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
        if (_currCell >= 0) then
        begin
          s := xml.Attributes.ItemsByName['hidden'];
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            ZXLSXGetColor(_dfFills[_dfIndex].patterncolor, _dfFills[_dfIndex].patternColorType, _dfFills[_dfIndex].lumFactorPattern);
      ...
      begin
        ...
        begin
          ...
          if (_tmptag = 'left') then
            _SetDFBorder(0)
          else if (_tmptag = 'right') then
            _SetDFBorder(2)
          else if (_tmptag = 'top') then
            _SetDFBorder(1)
          else if (_tmptag = 'bottom') then
            _SetDFBorder(3)
          else if (_tmptag = 'vertical') then
            _SetDFBorder(4)
          else if (_tmptag = 'horizontal') then
            _SetDFBorder(5)
          else if (_tmptag = 'color') then
          begin
      ...
      begin
        ...
        begin
          if (xml.TagName = 'font') then
            _ReadDFFont()
          else if (xml.TagName = 'fill') then
            _ReadDFFill()
          else if (xml.TagName = 'border') then
            _ReadDFBorder();
        end;
      end; //while
    end; //_ReaddxfItem
  ...
  try
    ...
    begin
      ...
      if ((xml.TagName = 'fonts') and (xml.TagType = TAG_TYPE_START)) then
        _ReadFonts()
      else if ((xml.TagName = 'borders') and (xml.TagType = TAG_TYPE_START)) then
        _ReadBorders()
      else if ((xml.TagName = 'fills') and (xml.TagType = TAG_TYPE_START)) then
        _ReadFills()
      else if ((xml.TagName = 'cellStyleXfs') and (xml.TagType = TAG_TYPE_START)) then
        _ReadCellCommonStyles('cellStyleXfs', CellStyleArray, CellStyleCount)//_ReadCellStyleXfs()
      else if ((xml.TagName = 'cellXfs') and (xml.TagType = TAG_TYPE_START)) then
        _ReadCellCommonStyles('cellXfs', CellXfsArray, CellXfsCount) //_ReadCellXfs()
      else if ((xml.TagName = 'cellStyles') and (xml.TagType = TAG_TYPE_START)) then //??
        _ReadCellStyles()
      else if ((xml.TagName = 'colors') and (xml.TagType = TAG_TYPE_START)) then
        _ReadColors()
      else if ((xml.TagType = TAG_TYPE_START) and (xml.TagName = 'dxfs')) then
        _Readdxfs()
      else if ((xml.TagType = TAG_TYPE_START) and (xml.TagName = 'numFmts')) then
...
begin
  ...
  try
    ...
    begin
      ...
      else if ((xml.TagName = 'workbookView') and (xml.TagType = TAG_TYPE_CLOSED)) then
      begin
  ...
  try
    ...
    begin
      ...
      if ((xml.TagName = 'authors') and (xml.TagType = TAG_TYPE_START)) then
        _ReadAuthors()
      else if ((xml.TagName = 'comment') and (xml.TagType = TAG_TYPE_START)) then
        _ReadComment();
...
var
  ...
  begin
    lst.Clear();
    lst.Add(ZH.FileArray[num].name); // file list
...
begin
  ...
  try
    try
      ...
      lst.Clear();
      lst.Add('[Content_Types].xml'); // file list
...
var
  ...
  begin
    ...
    b := (_SOptions.SplitVerticalMode <> ZSplitNone) or (_SOptions.SplitHorizontalMode <> ZSplitNone);
    if (b) then
    begin
      _xml.Attributes.Clear();
      _AddSplitValue(_SOptions.SplitVerticalMode, _SOptions.SplitVerticalValue, 'xSplit');
      _AddSplitValue(_SOptions.SplitHorizontalMode, _SOptions.SplitHorizontalValue, 'ySplit');

      _AddTopLeftCell(_SOptions.SplitVerticalMode, _SOptions.SplitVerticalValue, _SOptions.SplitHorizontalMode, _SOptions.SplitHorizontalValue);
  ...
  begin
    ...
    begin
      ...
      _xml.Attributes.Add('min', s, false);
      s := '0';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        b := (_sheet.Cell[j, i].Data > '') or (_sheet.Cell[j, i].Formula > '');
        ...
        begin
          ...
          if (_sheet.Cell[j, i].CellType = ZEString) and (_sheet.Cell[j, i].Formula = '') then
          begin
            _xml.Attributes.Clear();
            _xml.WriteTagNode('is', true, true, false);
            _xml.Attributes.Clear();
            _xml.WriteTag('t', _sheet.Cell[j, i].AsString, true, false, true);
            _xml.WriteEndTagNode();
          end
          else if (_sheet.Cell[j, i].CellType = ZEBoolean) then
          begin
            _xml.Attributes.Clear();
            if _sheet.Cell[j, i].AsBoolean then
              _xml.WriteTag('v', '1', true, false, true)
            else
              _xml.WriteTag('v', '0', true, false, true);
          end
          else if (_sheet.Cell[j, i].CellType = ZEDateTime) then
          begin
            _xml.Attributes.Clear();
            _xml.WriteTag('v', ZEFloatSeparator(FormatFloat('0.##########', _sheet.Cell[j, i].AsDateTime)), true, false, true);
          end
          else if (_sheet.Cell[j, i].Data > '') then
          begin
            _xml.Attributes.Clear();
...
//    _xml.Attributes.Add('orientation', s, false);
    ...
    _xml.Attributes.Add('orientation', IfThen(_sheet.SheetOptions.PortraitOrientation, 'portrait', 'landscape'), false);
...
var
  ...
  var
    ...
    begin
      ...
      //If cell type is datetime and cell style is empty then need write default NumFmtId = 14 for non-default styles.
      if ((Trim(_style.NumberFormat) = '') or (UpperCase(_style.NumberFormat) = 'GENERAL')) then
      begin
        if (StyleNum >= 0) and (_is_dateTime[StyleNum + 1]) then
          Result := 14
        else if (StyleNum >= 0) then
        begin
          b := false;
          _cs := _currSheet;
  ...
  procedure _UpdateArrayIndex(var arr: TIntegerDynArray; cnt: integer); // deprecated: remove CNT parameter in future version
  var
  ...
  begin
    result := (style1.BGColor = style2.BGColor) and (style1.PatternColor = style2.PatternColor) and (style1.CellPattern = style2.CellPattern);
  end; //_isFillsEqual
...
//        ZPDiagStripe:         s := 'darkUpDark'; ??
  ...
  begin
    ...
    _addalignment := _style.Alignment.WrapText or _style.Alignment.VerticalText or (_style.Alignment.Rotate <> 0) or (_style.Alignment.Indent <> 0) or _style.Alignment.ShrinkToFit or (_style.Alignment.Vertical <> ZVAutomatic) or (_style.Alignment.Horizontal <> ZHAutomatic);
    ...
    _xml.Attributes.Add('fillId', IntToStr(_FillIndex[NumStyle + 1] + 2), false);
...
function ZEXLSXCreateRelsWorkBook(PageCount: integer; Stream: TStream): integer;
var
  _xml: TZsspXMLWriterH;
  i: integer;
begin
...
function ExportXmlssToXLSX(var XMLSS: TZEXMLSS; FileName: string;
    const SheetsNumbers: array of integer; const SheetsNames: array of string;
    AllowUnzippedFolder: boolean = false): integer;
var
  _pages: TIntegerDynArray;
  _names: TStringDynArray;      // sheet names
  kol, i, ii: integer;
  zip: TZipper;
...
begin
  ...
  try
    ...
    begin
      ...
      ZEXLSXCreateSheet(XMLSS, TmpStream, _pages[i], _WriteHelper);
      ...
      begin
        ...
        _WriteHelper.CreateSheetRels(TmpStream);
      end;
    ...
    begin
      ...
      begin
        ...
        ZEXLSXCreateDrawing(XMLSS, TmpStream, _drawing);

        // drawings/_rels/drawingN.xml.rels
        TmpStream := _AddFile(path_draw_rel + 'drawing' + IntToStr(i + 1) + '.xml.rels');
        ZEXLSXCreateDrawingRels(XMLSS, TmpStream, _drawing);

        // media/imageN.png
        for ii := 0 to _drawing.PictureStore.Count - 1 do
        begin
          _pic := _drawing.PictureStore[ii];
          if not Assigned(_pic.DataStream) then
            Continue;
    ...
    ZEXLSXCreateWorkBook(XMLSS, TmpStream, _pages, _names, kol);

    //[Content_Types].xml
    TmpStream := _AddFile('[Content_Types].xml');
    ZEXLSXCreateContentTypes(XMLSS, TmpStream, kol, 0, nil, _WriteHelper);

    path_docprops := 'docProps/';
    // docProps/app.xml
    TmpStream := _AddFile(path_docprops + 'app.xml');
    ZEXLSXCreateDocPropsApp(TmpStream);

    // docProps/core.xml
    TmpStream := _AddFile(path_docprops + 'core.xml');
    ZEXLSXCreateDocPropsCore(XMLSS, TmpStream);
...
function SaveXmlssToXLSXPath(var XMLSS: TZEXMLSS; PathName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;
begin
  Result := ExportXmlssToXLSX(XMLSS, PathName, SheetsNumbers, SheetsNames, True);
end;
...
function SaveXmlssToXLSX(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer;
begin
  Result := ExportXmlssToXLSX(XMLSS, FileName, SheetsNumbers, SheetsNames, False);
end;
