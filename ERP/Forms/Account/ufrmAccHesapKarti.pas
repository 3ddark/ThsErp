...
type
  TfrmAccHesapKarti = class(TfrmBaseInputDB)
...
uses
  ...
  Ths.Database.Table.ChHesapKartiAra, ufrmAccHesapKartlariAra,
  Ths.Database.Table.ChGruplar, ufrmAccGruplar, Ths.Database.Table.PrsPersoneller,
  Ths.Database.Table.ChBolgeler, ufrmAccBolgeler,
...
procedure TfrmAccHesapKarti.cbbara_hesap_koduExit(Sender: TObject);
begin
  inherited;
  edthesap_kodu.Text := getHesapKodu;
end;

procedure TfrmAccHesapKarti.cbbson_hesap_koduExit(Sender: TObject);
begin
  inherited;
  edthesap_kodu.Text := getHesapKodu;
end;

procedure TfrmAccHesapKarti.edtkok_hesap_koduExit(Sender: TObject);
begin
  inherited;
  edthesap_kodu.Text := getHesapKodu;
end;

procedure TfrmAccHesapKarti.fillAraHesapNumbers;
var
  n1: Integer;
...
procedure TfrmAccHesapKarti.fillSonHesapNumbers(pKokHesap, pAraHesapKodu: string);
var
...
procedure TfrmAccHesapKarti.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmAccHesapKarti.FormDestroy(Sender: TObject);
begin
  FSetChHesapPlani.Free;
  inherited;
end;

procedure TfrmAccHesapKarti.FormShow(Sender: TObject);
begin
...
function TfrmAccHesapKarti.getHesapKodu: string;
begin
...
function TfrmAccHesapKarti.getAraHesapKodu: string;
begin
  Result := '';
...
procedure TfrmAccHesapKarti.HelperProcess(Sender: TObject);
var
  LFrmHesapKarti: TfrmAccHesapKartlariAra;
  LHesapKarti: TChHesapKartiAra;
  vKod: TStringDynArray;
  LFrmGrup: TfrmAccGruplar;
  LFrmBolge: TfrmAccBolgeler;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LFrmHesapKarti := TfrmAccHesapKartlariAra.Create(TEdit(Sender), Self, LHesapKarti, fomNormal, True);
        try
      ...
      begin
        LFrmGrup := TfrmAccGruplar.Create(TEdit(Sender), Self, TChGrup.Create(Table.Database), fomNormal, True);
        try
          LFrmGrup.ShowModal;
      ...
      begin
        LFrmBolge := TfrmAccBolgeler.Create(TEdit(Sender), Self, TChBolge.Create(Table.Database), fomNormal, True);
        try
          LFrmBolge.ShowModal;
...
procedure TfrmAccHesapKarti.pgcMainChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmAccHesapKarti.RefreshData();
var
...
procedure TfrmAccHesapKarti.Repaint;
begin
  inherited;
...
procedure TfrmAccHesapKarti.ShowHideMukellefTipi;
begin
...
function TfrmAccHesapKarti.ValidateInput(panel_groupbox_pagecontrol_tabsheet: TWinControl): Boolean;
...
procedure TfrmAccHesapKarti.btnAcceptClick(Sender: TObject);
begin
  if (FormMode = ifmNewRecord) or (FormMode = ifmCopyNewRecord) or (FormMode = ifmUpdate) then
  begin
