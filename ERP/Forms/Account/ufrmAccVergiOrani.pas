...
type
  TfrmAccVergiOrani = class(TfrmBaseInputDB)
...
uses
  Ths.Globals,
  ufrmAccHesapKartlari,
  Ths.Database.Table.SetChVergiOranlari;

{$R *.dfm}

procedure TfrmAccVergiOrani.FormShow(Sender: TObject);
begin
...
procedure TfrmAccVergiOrani.HelperProcess(Sender: TObject);
var
  LFrmHesap: TfrmAccHesapKartlari;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LFrmHesap := TfrmAccHesapKartlari.Create(TEdit(Sender), Self, TChHesapKarti.Create(Table.Database), fomNormal, True);
        try
          LFrmHesap.ShowModal;
...
procedure TfrmAccVergiOrani.RefreshData;
begin
...
procedure TfrmAccVergiOrani.btnAcceptClick(Sender: TObject);
begin
  if (FormMode = ifmNewRecord) or (FormMode = ifmCopyNewRecord) or (FormMode = ifmUpdate) then
  begin
