...
uses
  Winapi.Windows, System.SysUtils, System.Types, System.Classes, System.UITypes,
  Vcl.Grids, Vcl.Graphics, zexmlss, zsspxml, zesavecommon;
...
function XmlSSToGrid(var Grid: TStringGrid; var XMLSS: TZEXMLSS; const PageNum: integer; ToCol: integer; ToRow: integer; BCol, BRow, ECol, ERow: integer; InsertMode: byte; StyleCopy: integer = 511): boolean; overload;

function SaveXmlssToHtml(var XMLSS: TZEXMLSS; const PageNum: integer; Title: string; Stream: TStream; TextConverter: TAnsiToCPConverter; CodePageName: string): integer; overload;

function SaveXmlssToHtml(var XMLSS: TZEXMLSS; const PageNum: integer; Title: string; FileName: string; TextConverter: TAnsiToCPConverter; CodePageName: string): integer; overload;

function SaveXmlssToEXML(var XMLSS: TZEXMLSS; Stream: TStream; const SheetsNumbers: array of integer; const SheetsNames: array of string; TextConverter: TAnsiToCPConverter; CodePageName: string; BOM: ansistring = ''): integer; overload;

function SaveXmlssToEXML(var XMLSS: TZEXMLSS; Stream: TStream; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;

function SaveXmlssToEXML(var XMLSS: TZEXMLSS; Stream: TStream): integer; overload;

function SaveXmlssToEXML(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string; TextConverter: TAnsiToCPConverter; CodePageName: string; BOM: ansistring = ''): integer; overload;

function SaveXmlssToEXML(var XMLSS: TZEXMLSS; FileName: string; const SheetsNumbers: array of integer; const SheetsNames: array of string): integer; overload;
...
function SplitString(const buffer: string; const delimeter: char): TStringDynArray; overload;

implementation

uses
  zenumberformats, System.StrUtils, System.AnsiStrings;

function SplitString(const buffer: string; const delimeter: char): TStringDynArray;
begin
  Result := System.StrUtils.SplitString(buffer, delimeter); // implicit typecast
end;

function GridCheck(var XMLSS: TZEXMLSS; const PageNum: integer; GridIsNil: boolean; ToCol: integer; ToRow: integer; BCol, BRow, ECol, ERow: integer): boolean;
begin
  result := true;
  if XMLSS = nil then
  begin
...
function MainGridToXmlSS(var XMLSS: TZEXMLSS; const PageNum: integer; var Grid: TObject; ToCol: integer; ToRow: integer; BCol, BRow, ECol, ERow: integer; ignorebgcolor: boolean; _border: byte): boolean;
var
...
begin
  ...
  begin
    ...
    if (BCol >= sGrid.ColCount) or (ECol >= sGrid.ColCount) or (BRow >= sGrid.RowCount) or (ERow >= sGrid.RowCount) then
      result := false
    else
    begin
...
function GridToXmlSS(var XMLSS: TZEXMLSS; const PageNum: integer; var Grid: TStringGrid; ToCol: integer; ToRow: integer; BCol, BRow, ECol, ERow: integer; ignorebgcolor: boolean; _border: byte): boolean; overload;
begin
  result := MainGridToXmlSS(XMLSS, PageNum, TObject(Grid), ToCol, ToRow, BCol, BRow, ECol, ERow, ignorebgcolor, _border);
end;
  ...
  begin
    ...
    begin
      ...
      case Style.Border[i].LineStyle of
        ...
        ZEDot, ZEDashDotDot:
          s := s + ' ' + 'dotted';
        ZEDash, ZEDashDot, ZESlantDashDot:
          s := s + ' ' + 'dashed';
        ZEDouble:
          s := s + ' ' + 'double';
      else
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              {tut}
              t := XMLSS.Sheets[PageNum].MergeCells.Items[NumTopLeft].Right - XMLSS.Sheets[PageNum].MergeCells.Items[NumTopLeft].Left;
              if t > 0 then
                Attributes.Add('colspan', InttOstr(t + 1));
              t := XMLSS.Sheets[PageNum].MergeCells.Items[NumTopLeft].Bottom - XMLSS.Sheets[PageNum].MergeCells.Items[NumTopLeft].Top;
              if t > 0 then
            ...
            else if XMLSS.Styles[t].Alignment.Horizontal = ZHRight then
              Attributes.Add('align', 'right')
            else if XMLSS.Styles[t].Alignment.Horizontal = ZHJustify then
...
const
  RepeatablePrintedHeadersName = 'Print_Titles';
...
//      WriteTag('Version', ZELibraryVersion); // Excel 2010 writes "14.00" here despite specifications say Integer
  ...
  begin
    ...
    begin
      ...
      if not (((_border[i].LineStyle = ZEContinuous) or (_border[i].LineStyle = ZEHair)) and (_border[i].Weight = 0)) then
      begin
        case i of
          0:
            _xml.Attributes.Add('ss:Position', 'Left', false);
          1:
            _xml.Attributes.Add('ss:Position', 'Top', false);
          2:
            _xml.Attributes.Add('ss:Position', 'Right', false);
          3:
            _xml.Attributes.Add('ss:Position', 'Bottom', false);
          4:
            _xml.Attributes.Add('ss:Position', 'DiagonalLeft', false);
          5:
            _xml.Attributes.Add('ss:Position', 'DiagonalRight', false);
        end;
        NonReqBorderAttr(i);
      end;
  ...
  begin
    ...
    begin
      ...
      // Rotate
      AddAttribute('ss:Rotate', zeNormalizeAngle90(_style.Alignment.Rotate), 0, zeNormalizeAngle90(XMLSS.Styles.DefaultStyle.Alignment.Rotate), _def);
      // ShrinkToFit
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        t := Length(s);
        if s[t] = ',' then
          SetLength(s, t - 1); // no rows if ends with comma
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              t := ProcessedSheet.MergeCells.Items[NumTopLeft].Right - ProcessedSheet.MergeCells.Items[NumTopLeft].Left;
              if t > 0 then
                AttrCell.Add('ss:MergeAcross', InttOstr(t), false);

              t := ProcessedSheet.MergeCells.Items[NumTopLeft].Bottom - ProcessedSheet.MergeCells.Items[NumTopLeft].Top;
              if t > 0 then
            ...
            //StyleID
            if (ProcessedCell.CellStyle <> -1) and (ProcessedCell.CellStyle < XMLSS.Styles.Count) then
            ...
            if (ProcessedSheet.RowsToRepeat.Active and (ProcessedSheet.RowsToRepeat.From <= i) and (ProcessedSheet.RowsToRepeat.Till >= i)) or (ProcessedSheet.ColsToRepeat.Active and (ProcessedSheet.ColsToRepeat.From <= j) and (ProcessedSheet.ColsToRepeat.Till >= j)) then
            begin
              inc(kol);
            ...
            if (ProcessedCell.Data > '') or (j = ProcessedSheet.ColCount - 1) or (isFormula) then
            begin
            ...
            begin
              ...
              //Data
              if (ProcessedCell.Data > '') or (isFormula) then
              begin
      ...
      begin
        ...
        WriteTag('HorizontalResolution', '600', true, false, false);
      ...
      b := (_vMode <> ZSplitNone) or (_hMode <> ZSplitNone);
      if (b) then
        b := (ProcessedSheet.SheetOptions.SplitVerticalValue <> 0) or (ProcessedSheet.SheetOptions.SplitHorizontalValue <> 0);

      if (b) then
      begin
        b := (_vMode = ZSplitFrozen) or (_hMode = ZSplitFrozen);
        if (b) then
        begin
//          WriteEmptyTag('Selected', true, false);
        ...
        _WriteSplitFrozen(_xml, _hMode, ProcessedSheet.SheetOptions.SplitHorizontalValue, 'SplitHorizontal', 'TopRowBottomPane');

        _WriteSplitFrozen(_xml, _vMode, ProcessedSheet.SheetOptions.SplitVerticalValue, 'SplitVertical', 'LeftColumnRightPane');

        s := '';
        if ((_hMode = ZSplitFrozen) and (_vMode = ZSplitFrozen)) then
          s := '0'
        else if (_hMode = ZSplitFrozen) then
          s := '2'
        else if (_vMode = ZSplitFrozen) then
          s := '1';
      ...
      if not ((ProcessedSheet.SheetOptions.ActiveCol = 0) and (ProcessedSheet.SheetOptions.ActiveRow = 0)) then
      begin
...
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Alignment.Rotate := TZCellTextRotate(_StrToInt(_xml.Attributes.ItemsByName['ss:Rotate']));
        ...
        begin
          ...
          if (s > '') then
            font.Charset := TFontCharset(_StrToInt(s));
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (s > '') then
                  Border[i].Weight := Byte(_StrToInt(s));
              end
            end;
          end
        else
    ...
    begin
      ...
      begin
        if _xml.TagName = 'Author' then
          XMLSS.DocumentProperties.Author := trim(_xml.TextBeforeTag)
        else if _xml.TagName = 'LastAuthor' then
          XMLSS.DocumentProperties.LastAuthor := trim(_xml.TextBeforeTag)
        else if _xml.TagName = 'Created' then
        begin
          {tut}
          //XMLSS.DocumentProperties.Created :=
        end
        else if _xml.TagName = 'Company' then
          XMLSS.DocumentProperties.Company := trim(_xml.TextBeforeTag)
        else if _xml.TagName = 'Version' then
    ...
    begin
      ...
      begin
        if _xml.TagName = 'WindowHeight' then
          XMLSS.DocumentProperties.WindowHeight := Word(_StrToInt(trim(_xml.TextBeforeTag)))
        else if _xml.TagName = 'WindowWidth' then
          XMLSS.DocumentProperties.WindowWidth := Word(_StrToInt(trim(_xml.TextBeforeTag)))
        else if _xml.TagName = 'WindowTopX' then
          XMLSS.DocumentProperties.WindowTopX := Word(_StrToInt(trim(_xml.TextBeforeTag)))
        else if _xml.TagName = 'WindowTopY' then
          XMLSS.DocumentProperties.WindowTopY := Word(_StrToInt(trim(_xml.TextBeforeTag)));
      end
      else if _xml.TagType = 5 then
  ...
  var
    ...
    begin
      result := SplitValue;
      if (SplitMode = ZSplitSplit) then
        result := Integer(round(PointToPixel(SplitValue / 20)));
    end; //_GetSplitValue
        ...
        begin
          ...
          begin
            ...
            if _xml.Attributes.ItemsByName['x:CenterHorizontal'] = '1' then
              _SheetOptions.CenterHorizontal := true
            else if _xml.Attributes.ItemsByName['x:CenterVertical'] = '1' then
              _SheetOptions.CenterVertical := true
            else
            begin
          ...
          else if IfTag('PageMargins', 5) then
          begin
            s := _xml.Attributes.ItemsByName['x:Bottom'];
            if (s > '') then
              _SheetOptions.MarginBottom := Word(round(ZETryStrToFloat(s) * 25.4));
            s := _xml.Attributes.ItemsByName['x:Left'];
            if (s > '') then
              _SheetOptions.MarginLeft := Word(round(ZETryStrToFloat(s) * 25.4));
            s := _xml.Attributes.ItemsByName['x:Right'];
            if (s > '') then
              _SheetOptions.MarginRight := Word(round(ZETryStrToFloat(s) * 25.4));
            s := _xml.Attributes.ItemsByName['x:Top'];
            if (s > '') then
              _SheetOptions.MarginTop := Word(round(ZETryStrToFloat(s) * 25.4));
          end
          else if IfTag('Header', 5) then
          begin
            s := _xml.Attributes.ItemsByName['x:Margin'];
            if (s > '') then
              _SheetOptions.HeaderMargins.Height := Word(abs(round(ZETryStrToFloat(s) * 25.4)));
            _SheetOptions.HeaderData := _xml.Attributes.ItemsByName['x:Data'];
          end
          else if IfTag('Footer', 5) then
          begin
            s := _xml.Attributes.ItemsByName['x:Margin'];
            if (s > '') then
              _SheetOptions.FooterMargins.Height := Word(abs(round(ZETryStrToFloat(s) * 25.4)));
      ...
      //Print
        ...
        begin
          ...
          if IfTag('PaperSizeIndex', 6) then
            _SheetOptions.PaperSize := Byte(_StrToInt(trim(_xml.TextBeforeTag)));
        end
      else if IfTag('Selected', 5) then
        XMLSS.Sheets[PageNum].Selected := true
      else
      //Panes
          ...
          if IfTag('Pane', 4) then
            ...
            begin
              ...
              if IfTag('ActiveRow', 6) then
                _SheetOptions.ActiveRow := Word(_StrToInt(trim(_xml.TextBeforeTag)))
              else if IfTag('ActiveCol', 6) then
                _SheetOptions.ActiveCol := Word(_StrToInt(trim(_xml.TextBeforeTag)));
            end;
        end
      else if IfTag('TabColorIndex', 6) then
        XMLSS.Sheets[PageNum].TabColor := _StrToInt(trim(_xml.TextBeforeTag))
      else
      //Spli/Frozen
        if (IfTag('FreezePanes', 5)) then
        _isFreezePanes := true
      else if (IfTag('FrozenNoSplit', 5)) then
        _isFrozeNoSplit := true
      else if (IfTag('SplitHorizontal', 6)) then
      begin
        _isH := true;
        s := _xml.TextBeforeTag;
        _SheetOptions.SplitHorizontalMode := ZSplitSplit;
        _SheetOptions.SplitHorizontalValue := Integer(round(ZETryStrToFloat(s)));
      end
      else if (IfTag('SplitVertical', 6)) then
      begin
        _isV := true;
        s := _xml.TextBeforeTag;
        _SheetOptions.SplitVerticalMode := ZSplitSplit;
        _SheetOptions.SplitVerticalValue := Integer(round(ZETryStrToFloat(s)));
      end;
    end; //while
  ...
  begin
    ...
    begin
      ...
      if IfTag('Table', 4) then
        ReadXMLTable(num)
      else if IfTag('WorksheetOptions', 4) then
        ReadXMLWorkSheetOptions(num)
      else if IfTag('PageBreaks', 4) then
        ReadPageBreaks(num);
    end;
  end;
