...
begin
  ...
  stime := '_tmp' + IntToStr(_y) + IntToStr(_m) + IntToStr(_d) + IntToStr(_h) + IntToStr(_min) + IntToStr(_s) + IntToStr(_ms) + '_';
  s := '';
  kol := 0;
  repeat
...
function ZEDelTree(ADir: string): boolean;
  ...
  begin
    result := true;
    if FindFirst(AddDir + '*.*', faAnyFile, sr) = 0 then
    try
      repeat
        if ((sr.Attr and faDirectory) = faDirectory) then
        begin
          if (sr.Name <> '..') and (sr.Name <> '.') then
            _DelTree(AddDir + sr.Name + PathDelim);
        end
        else
          result := result and DeleteFile(AddDir + sr.Name);
      until FindNext(sr) <> 0;
    finally
      FindClose(sr);
      if (result) then
        result := result and RemoveDir(AddDir);
    end;
  end;
...
begin
  ...
  inherited Create(CreateFile(PChar(FileName), GENERIC_READ or GENERIC_WRITE, 0, nil, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY or FILE_FLAG_DELETE_ON_CLOSE, 0));
  if Handle < 0 then
