unit Ths.Database.Table.EmpPersonnel;
...
uses
  ...
  Ths.Database,
  Ths.Database.Table,
  Ths.Database.Table.EmpPersonelTipi,
  Ths.Database.Table.EmpBolum,
  Ths.Database.Table.EmpBirim,
  Ths.Database.Table.EmpGorev,
  Ths.Database.Table.EmpTasimaServisi,
  Ths.Database.Table.SysAdresler,
  Ths.Database.Table.SysUlkeler,
  Ths.Database.Table.SysSehirler,
  Ths.Database.Table.EmpDriverLicence;
...
type
  ...
  private
    ...
    FEmpPersonelTipi: TEmpPersonelTipi;
    FEmpBolum: TEmpBolum;
    FEmpBirim: TEmpBirim;
    FEmpGorev: TEmpGorev;
    FEmpTasimaServisi: TEmpTasimaServisi;
    FSysUlke: TSysUlke;
    FSysSehir: TSysSehir;
  protected
...
constructor TEmpPersonnel.Create(ADatabase: TDatabase);
begin
  TableName := 'emp_person';
  ...
  FEmpPersonelTipi := TEmpPersonelTipi.Create(Database);
  FEmpBolum := TEmpBolum.Create(Database);
  FEmpBirim := TEmpBirim.Create(Database);
  FEmpGorev := TEmpGorev.Create(Database);
  FEmpTasimaServisi := TEmpTasimaServisi.Create(Database);
  ...
  FPersonelTipi := TFieldDB.Create(FEmpPersonelTipi.PersonelTipi.FieldName, FEmpPersonelTipi.PersonelTipi.DataType, FEmpPersonelTipi.PersonelTipi.Value, Self);
  ...
  FBirim := TFieldDB.Create(FEmpBirim.Birim.FieldName, FEmpBirim.Birim.DataType, '', Self);
  FGorevID := TFieldDB.Create('gorev_id', ftInteger, 0, Self);
  FGorev := TFieldDB.Create(FEmpGorev.Gorev.FieldName, FEmpGorev.Gorev.DataType, '', Self);
  ...
  FTasimaServis := TFieldDB.Create(FEmpTasimaServisi.AracAdi.FieldName, FEmpTasimaServisi.AracAdi.DataType, '', Self);
...
destructor TEmpPersonnel.Destroy;
begin
  Adres.Free;

  FEmpPersonelTipi.Free;
  FEmpBolum.Free;
  FEmpBirim.Free;
  FEmpGorev.Free;
  FEmpTasimaServisi.Free;
  FSysUlke.Free;
  FSysSehir.Free;
...
function TEmpPersonnel.SelectToDatasource(AFilter: string; APermissionControl: Boolean; AAllColumn: Boolean; AHelper: Boolean): string;
var
  LQry: TFDQuery;
begin
  ...
  try
    Database.SQLBuilder.GetSQLSelectCmd(LQry, TableName, [
      ...
      addField(FEmpPersonelTipi.TableName, FEmpPersonelTipi.PersonelTipi.FieldName, FPersonelTipi.FieldName),
      addField(FEmpBolum.TableName, FEmpBolum.Bolum.FieldName, FBolum.FieldName),
      FBirimID.QryName,
      addField(FEmpBirim.TableName, FEmpBirim.Birim.FieldName, FBirim.FieldName),
      FGorevID.QryName,
      addField(FEmpGorev.TableName, FEmpGorev.Gorev.FieldName, FGorev.FieldName),
      ...
      addField(FEmpTasimaServisi.TableName, FEmpTasimaServisi.AracAdi.FieldName, FTasimaServis.FieldName),
      FOzelNot.QryName,
      FMaas.QryName,
      ...
      addJoin(jtLeft, FEmpPersonelTipi.TableName, FEmpPersonelTipi.Id.FieldName, TableName, FPersonelTipiID.FieldName),
      addJoin(jtLeft, FEmpBirim.TableName, FEmpBirim.Id.FieldName, TableName, FBirimID.FieldName),
      addJoin(jtLeft, FEmpBolum.TableName, FEmpBolum.Id.FieldName, FEmpBirim.TableName, FEmpBirim.BolumID.FieldName),
      addJoin(jtLeft, FEmpGorev.TableName, FEmpGorev.Id.FieldName, TableName, FGorevID.FieldName),
      addJoin(jtLeft, FEmpTasimaServisi.TableName, FEmpTasimaServisi.Id.FieldName, TableName, FTasimaServisID.FieldName),
      ' WHERE 1=1 ', AFilter
    ], AAllColumn, AHelper);
...
procedure TEmpPersonnel.SelectToList(AFilter: string; ALock: Boolean; APermissionControl: Boolean);
var
  LQry: TFDQuery;
begin
  if not IsAuthorized(ptRead, APermissionControl) then
...
procedure TEmpPersonnel.DoInsert(APermissionControl: Boolean);
var
  LQry: TFDQuery;
begin
  LQry := Database.NewQuery();
...
procedure TEmpPersonnel.DoUpdate(APermissionControl: Boolean);
var
  LQry: TFDQuery;
begin
  LQry := Database.NewQuery();
...
procedure TEmpPersonnel.BusinessSelect(AFilter: string; ALock, APermissionControl: Boolean);
var
  AEmpPersonnel: TEmpPersonnel;
  n1: Integer;
begin
  Self.SelectToList(AFilter, ALock, APermissionControl);
  for n1 := 0 to List.Count-1 do
  begin
    AEmpPersonnel := TEmpPersonnel(Self.List[n1]);
    if AEmpPersonnel.AdresID.AsInt64 > 0 then
      AEmpPersonnel.Adres.SelectToList(' AND ' + AEmpPersonnel.Adres.Id.QryName + '=' + AEmpPersonnel.AdresID.AsString, ALock, False)
    else
      AEmpPersonnel.Adres.Clear;
  end;
end;

procedure TEmpPersonnel.BusinessInsert(APermissionControl: Boolean);
begin
...
procedure TEmpPersonnel.BusinessUpdate(APermissionControl: Boolean);
var
  LImgFileName: string;
begin
...
procedure TEmpPersonnel.BusinessDelete(APermissionControl: Boolean);
begin
  inherited;
//
end;

function TEmpPersonnel.Clone():TTable;
begin
  Result := TEmpPersonnel.Create(Database);
  CloneClassContent(Self, Result);
  CloneClassContent(Self.Adres, TEmpPersonnel(Result).Adres);
end;
