...
begin
  ...
  FUlkeID := TFieldDB.Create('ulke_id', FSysSehir.UlkeID.DataType, 0, Self, 'Ülke Adı ID');
  FUlke := TFieldDB.Create(FSysSehir.UlkeAdi.FieldName, FSysSehir.UlkeAdi.DataType, '', Self, 'Ülke');
  FBolgeID := TFieldDB.Create('bolge_id', FSysSehir.BolgeID.DataType, 0, Self, 'Bölge Adı ID');
  ...
  FKacYillikFirma := TFieldDB.Create('kac_yillik_firma', ftInteger, 0, Self, 'Kaç Yıllık Firma');
  FCalismaDurumuId := TFieldDB.Create('calisma_durumu_id', ftInteger, 0, Self, 'Çalışma Durumu ID');
  FCalismaDurumu := TFieldDB.Create(FBbkCalismaDurumu.CalismaDurumu.FieldName, FBbkCalismaDurumu.CalismaDurumu.DataType, '', Self, 'Çalışma Durumu');
