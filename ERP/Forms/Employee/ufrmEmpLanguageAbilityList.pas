...
begin
  ...
  if (pFormMode = ifmRewiev) then
    Result := TEmpLanguageAbilityForm.Create(Application, Self, Table.Clone(), pFormMode)
  else if (pFormMode = ifmNewRecord) then
    Result := TEmpLanguageAbilityForm.Create(Application, Self, TEmpLanguageAbility.Create(Table.Database), pFormMode)
