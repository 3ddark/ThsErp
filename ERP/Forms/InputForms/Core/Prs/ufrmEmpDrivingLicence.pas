...
type
  TfrmEmpDrivingLicence = class(TfrmBaseInputDB)
...
procedure TfrmEmpDrivingLicence.btnAcceptClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      TPrsPersonelEhliyeti(Table).PersonelID.Value := TEmpEmployee(cbbemp_card_id.Items.Objects[cbbemp_card_id.ItemIndex]).Id.Value;
...
procedure TfrmEmpDrivingLicence.FormCreate(Sender: TObject);
var
  n1: Integer;
begin
  inherited;
...
procedure TfrmEmpDrivingLicence.FormDestroy(Sender: TObject);
begin
...
procedure TfrmEmpDrivingLicence.FormPaint(Sender: TObject);
begin
  inherited;
  cbbemp_card_id.Enabled := False;
end;

procedure TfrmEmpDrivingLicence.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TfrmEmpDrivingLicence.RefreshData;
var
  n1: Integer;
begin
  ...
  begin
    if Assigned(cbbemp_card_id.Items.Objects[n1]) then
      if TEmpEmployee(cbbemp_card_id.Items.Objects[n1]).Id.Value = TPrsPersonelEhliyeti(Table).PersonelID.Value then
      begin
...
procedure TfrmEmpDrivingLicence.Repaint;
begin
  inherited;
  //cbbemp_card_id.Enabled := False;
end;
