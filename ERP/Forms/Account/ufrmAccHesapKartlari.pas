...
type
  TfrmAccHesapKartlari = class(TfrmBaseDBGrid)
  protected
...
function TfrmAccHesapKartlari.CreateInputForm(Sender: TObject; pFormMode: TInputFormMode): TForm;
begin
  Result := nil;
  if (pFormMode = ifmRewiev) then
    Result := TfrmAccHesapKarti.Create(Application, Self, Table.Clone(), pFormMode)
  else if (pFormMode = ifmNewRecord) then
    Result := TfrmAccHesapKarti.Create(Application, Self, TChHesapKarti.Create(Table.Database), pFormMode)
  else if (pFormMode = ifmCopyNewRecord) then
    Result := TfrmAccHesapKarti.Create(Application, Self, Table.Clone(), pFormMode);
end;

procedure TfrmAccHesapKartlari.FormShow(Sender: TObject);
var
  LPasifFilter: string;
begin
  LPasifFilter := '';
