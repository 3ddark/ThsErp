...
type
  TEmpPersonnelListForm = class(TfrmBaseDBGrid)
...
function TEmpPersonnelListForm.CreateInputForm(Sender: TObject; pFormMode: TInputFormMode): TForm;
begin
  Result := nil;
  if (pFormMode = ifmRewiev) then
    Result := TEmpPersonelForm.Create(Self, Self, Table.Clone(), pFormMode)
  else if (pFormMode = ifmNewRecord) then
    Result := TEmpPersonelForm.Create(Self, Self, TEmpPersonnel.Create(Table.Database), pFormMode)
  else if (pFormMode = ifmCopyNewRecord) then
    Result := TEmpPersonelForm.Create(Self, Self, Table.Clone(), pFormMode);
end;

procedure TEmpPersonnelListForm.FormShow(Sender: TObject);
begin
  inherited;
